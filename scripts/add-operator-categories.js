...
async function processOperatorFiles(
  ...
  for (const item of items) {
    ...
    if (item.isDirectory()) {
      ...
      const results = await processOperatorFiles(
        fullPath,
        updatedCount,
        skippedCount,
        uncategorizedCount,
      ...
      console.warn(
        `✅ Added categories "${categories.join(", ")}" to ${operatorName}`,
      );
      updatedCount++;
    }
  }
...
async function addCategoriesToOperators() {
  ...
  console.warn(
    `   Skipped: ${results.skippedCount} files (already had category)`,
  );
