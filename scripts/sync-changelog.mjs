...
/**
 * Generate changelog landing page data.
 */
function generateLandingDataFile(latestTimelineYear, projectCards) {
  return `// This file is auto-generated by scripts/sync-changelog.mjs
// Do not edit manually.

export const latestTimelineYear = ${JSON.stringify(latestTimelineYear)};

export const changelogLandingProjects = ${JSON.stringify(projectCards, null, 2)};
`;
}
...
/**
 * Remove stale feed files that are no longer generated.
 */
async function removeStaleFeeds(outputDir, generatedFeeds) {
  const validFeeds = new Set(generatedFeeds);
...
async function syncChangelog(newsRepoPath) {
  ...
  // Clean stale project directories before regenerating
  // Preserve: index.mdx (git-tracked landing page shell, not regenerated)
  // Preserve: timeline/ (generated separately at end of sync)
  const existingEntries = await fs.readdir(changelogContentDir, {
    withFileTypes: true,
  });
  const validProjectIds = new Set(projects.map((p) => p.id));

  for (const entry of existingEntries) {
    // Skip non-directories (includes the tracked landing page shell)
    if (!entry.isDirectory()) {
      continue;
    }
    // Skip timeline directory (regenerated separately later in sync)
    if (entry.name === "timeline") {
      continue;
    }
    // Remove project directories not in current news repo
    if (!validProjectIds.has(entry.name)) {
      const stalePath = path.join(changelogContentDir, entry.name);
      console.log(`  Removing stale project: ${entry.name}`);
...
`;
  ...
  // Generate main changelog landing page data
  ...
  const projectCards = projects
    .map((project) => {
      ...
      return {
        ...
        icon: changelogProjects[project.id]?.icon || "document",
        color: changelogProjects[project.id]?.color,
        meta: version,
      };
    })
    .filter((project) => project.title);
  const landingDataPath = path.join(srcDir, "changelog-landing.generated.ts");
  await fs.writeFile(
    landingDataPath,
    generateLandingDataFile(latestTimelineYear, projectCards),
  );
  ...
  await removeStaleFeeds(feedsDir, generatedFeeds);
  console.log(
