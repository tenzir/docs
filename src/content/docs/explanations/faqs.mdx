...
1. **Executor**: The pipeline executor is open source and available under a
   permissive BSD 3-clause license at
...
yourself and [add your own plugins](/explanations/deployment), or use our
...
Read more on [how Tenzir works](/explanations/deployment) to understand what
...
We opted for our own language—the [**Tenzir Query Language**](/explanations/language) or **TQL**— for
...
files, accessible via the <Op>import</Op> and

<Op>export</Op> operators.
...
3. **Fluent Bit**. Sometimes we can compensate for the lack of an existing
   ...
   because the Fluent Bit library is baked into every Tenzir binary. Use Fluent
   Bit with <Op>from_fluent_bit</Op> to get events in and
   <Op>to_fluent_bit</Op> to send events out.
4. **Escape Hatches**. As last resort, you can bring in Shell and Python scripts
   to make up for native support for _X_. Use <Op>shell</Op> to bring byte
   streams via standard input and output into a pipeline, and use
   <Op>python</Op> for arbitrary event-to-event transformation in Python.
