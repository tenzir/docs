...
The Tenzir Query Language (TQL) is a dataflow language designed specifically for
security and network data analysis. This page explains how TQL works as a
complete language system, from its foundational concepts to advanced composition
patterns.
...
- **void**: No data (used for pipeline boundaries)
...
- **Control flow**: Route data conditionally ([`if`](/reference/operators/if) statements)
...
```tql
where event_id? != null        // Hides missing required data
