...
TQL **programs** compose [statements](/explanations/language/statements) into
...
semantics. For example, the [`every`](/reference/operators/every) operator
...
Some operators require that you define a closed (void-to-void) pipeline, whereas
others exhibit parsing (bytes-to-events) or printing (events-to-bytes)
semantics.

## Parallel processing

The [`fork`](/reference/operators/fork) operator splits a pipeline into multiple
parallel legs. It takes a nested pipeline as an argument that represents the
secondary leg.
