...
Each type in TQL provides specific operations, starting from the simplest and
building up to more complex types. For functions that work with these types,
see the [functions reference](/reference/functions).

### None

The `none` type represents absent or invalid values using the literal `null`:
...
TQL implements _short-circuit evaluation_: it stops evaluating once it determines
the result.
...
Format strings provide a concise way to build dynamic strings using embedded
expressions. They're much more readable than string concatenation. For example,
instead of:
...
When mixing numeric types, TQL automatically coerces to the type that can hold
the most values:
...
TQL handles numeric errors gracefully:
...
Write dates and timestamps using the [ISO 8601
...
Calculating elapsed time is a common operation that converts two time points into
a duration via subtraction:
...
This also applies to subnets: both `10.0.0.0/8` and `2001:db8::/32` are valid
subnets.
...
[Secrets](/explanations/secrets) protect sensitive values like authentication
tokens and passwords. The `secret` type contains only a secret's name, not its
actual value, which is resolved asynchronously when needed.

Create secrets using the [`secret`](/reference/functions/secret) function or
pass string literals directly to operators that accept secrets:
...
Secrets support concatenation with `+` and can be used in format strings. When
a format string contains a secret, the result is also a secret. Converting a
secret to a string yields a masked value (`"***"`) to prevent accidental exposure.

### List

TQL has _typed_ lists, which means that the type of the elements in a list is
fixed and must not change per element. Lists use brackets to sequence data. `[]`
denotes the empty list. Specify items with comma-delimited expressions:
...
Records use braces to structure data. `{}` denotes the empty record. Specify
fields using identifiers followed by a colon and an expression. Use quoted
strings for invalid field names. For example:
...
Records support field access with `.` and bracket notation:
...
The optional field access operator (`?`) suppresses warnings when accessing
non-existent fields:

```tql title="Processing events with optional fields"
...
{event: "logon", user_id: 456, name: null}  // null instead of warning
{event: "logoff", user_id: 123, name: null} // null instead of warning
```
...
from {
  ...
  city: user.address?.city?,  // Safe even if address doesn't exist
}
```
...
Without `else`, the `?` operator returns `null` when the field doesn't exist.
With `else`, you get a sensible default value instead:
...
Both indexing expressions and the [`get`](/reference/functions/get) function
(see next section below) support numeric indices for records:
...
The [`get`](/reference/functions/get) function provides safe access with
fallback values:

```tql
let $config = {timeout: 30}
retries = $config.get("retries", 3)  // Returns 3 if "retries" doesn't exist
```

To suppress warnings when the list index is out of bounds, use
[`get`](/reference/functions/get) with a fallback value:

```tql
let $my_list = ["Hello", "World"]
result = $my_list.get(2, "default")
```
...
To suppress warnings when the record field is missing, use
[`get`](/reference/functions/get) with a fallback value:
...
The `move` expression transfers a field's value and removes the original field
in one atomic operation. Use the `move` keyword in front of a field to relocate
...
In addition to the `move` keyword, there exists a
[`move`](/reference/operators/move) operator that is a convenient alternative
when relocating multiple fields. For example, this
...
can be rewritten succinctly with the [`move`](/reference/operators/move)
operator:
...
- Different from the [`move`](/reference/operators/move) operator that is a
  statement
  :::
...
prefix. For instance, `@name` holds the name of the event.

Currently, available metadata fields include `@name`, `@import_time`, and
`@internal`. Future updates may allow defining custom metadata fields.
...
Binary operators work on two operands. The supported operations depend on the
data types involved.
...
All types support equality comparison (`==`, `!=`). Additionally, ordered types
...
Expressions like `1 - 2 * 3 + 4` follow these precedence and associativity rules.
The expression evaluates as `(1 - (2 * 3)) + 4`. Example: `1 + 2 * 3` evaluates as `1 + (2 * 3)` = 7
...
where `x`, `y`, and `condition` are expressions.
...
Functions and take positional and/or named arguments, producing a value as a
result of their computation.
...
Some operators and functions accept **lambda expressions** of the form
`arg => expr`:
...
If the pipeline expression is the last argument, omit the preceding comma.
Braces can contain multiple statements separated by newlines.
...
:::caution[Evaluation Time]
Constants evaluate _once_ at pipeline start and remain available throughout.
:::
...
A **constant expression** evaluates to a constant when the pipeline containing
it starts. Many pipeline operators require constant arguments:
...
Type inference happens automatically, and null values propagate through
operations according to each operation's semantics.
