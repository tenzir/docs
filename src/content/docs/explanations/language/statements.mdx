...
3. **Delimiters**: Inside `()`, `[]`, or `{}`, newlines are allowed freely

A newline immediately after an operator name ends the statement, so `select`
followed by a newline is a complete (albeit invalid) statement.
