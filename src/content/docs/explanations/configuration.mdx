...
In particular, this applies to lists. For example, `TENZIR_PLUGINS="foo,bar"` is
equivalent to `--plugins=foo,bar`.
...
and <Op>to_splunk</Op>) and those that accept inbound connections
(e.g., <Op>accept_tcp</Op> and <Op>serve_tcp</Op>).
...
You can get the list of available plugins using the <Op>plugins</Op> operator:
