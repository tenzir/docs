...
directory as a package and contains descriptive fields, top-level categories,
external metadata, context definitions, and input specifications.
...
Packages that normalize to OCSF should expose mapping as operators too, usually
with a dispatcher such as `vendor::product::ocsf::map` and event-specific
operators under `operators/.../ocsf/events/`.
...
Ship optional ingestion, export, and publishing pipelines with `disabled: true`
so installation remains predictable.
...
Top-level `categories` describe where the package appears in the Tenzir
Library. Valid values are `sources`, `destinations`, `mappings`, and
`contexts`.
