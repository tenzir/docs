...
The `test` directory contains deterministic integration tests, primarily for
...
e.g., `from_file "{{ inputs.filename }}"`. Tenzir replaces the templates with
their configured values when installing the package.

To provide non-default values for the defined inputs, use the `config.inputs`
key:
...
You can use the `config.overrides` object to change the value of any field in
the package definition. This allows you to customize fields like `disabled` or
