...
[new-discussion]:
https://github.com/orgs/tenzir/discussions/new?category=questions-answers
...
A connector is usually exposed as a source operator that reads from a resource,
or as a sink operator that writes to a resource.
...
An pipeline ending with an [output](#output) operator preceded by
a <Op>subscribe</Op> input operator.
...
using the <Op>import</Op> and <Op>export</Op> [operators](#operator). The
storage cengine comes with a [catalog](#catalog) that tracks
[partitions](#partition) and keeps sparse [indexes](#index) to accelerate
historical queries.
...
system](/reference/types) that also has timestamps, durations, IP addresses, and
more. Events have fields and can contain numerous shapes that describe its types
(= the [schema](#schema)).
...
Tenzir featres in-memory _sparse_ indexes that point to
...
A source is the dual to a [sink](#sink). It has no input and emits events or
...
- <Guide>edge-storage/export-from-a-node</Guide>
...
An operator is an [input](#input), a [transformation](#transformation), or an
[output](#output).
...
A parser is the dual to a [printer](#printer). You use parsers via the `read_*`
...
A printer is the dual to a [parser](#parser). Use printers via the `write_*`
operators or as printing subpipelines in output operators.
...
A sink is the dual to a [source](#source). It consumes events or bytes from the
pipeline and has no output.
...
An pipeline starting with an [input](#input) operator followed by
a <Op>publish</Op> output operator.
