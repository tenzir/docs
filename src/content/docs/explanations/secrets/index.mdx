...
```tql
secret("my-encoded-secret").decode_base64()
```
...
### Secrets in Escape Hatches

Tenzir has two prominent escape hatch operators: [`shell`](/reference/operators/shell)
and [`python`](/reference/operators/python). By default, you cannot use secrets
in their respective scripts. If you want to use secret values in their scripts,
you can enable the configuration option `tenzir.allow-secrets-in-escape-hatches`.

This will resolve secrets on invocation of the script. The `python` operator
requires the script as plain text for this.

#### Python

Since secrets can be values in a pipeline, they can also be passed through the
`python` operator. You must not modify a secret in the python operator.
