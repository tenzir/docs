...
- By using the [`secret`](/reference/functions/secret) function ([Manged Secret](#managed-secrets)):
...
Secrets are a special type in Tenzir's type system. The `secret` type only
supports a limited set of operations, all of which yield `secret`.
...
is additionally encrypted using ECIES with a one-time, per secret key.
The value stays encrypted through the entire transfer until the final usage
site.
