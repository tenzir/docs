...
The following examples use the <Op>to_splunk</Op>
...
- Use the <Fn>secret</Fn> function ([Managed Secret](#managed-secrets)):
...
Tenzir's [type system](/reference/types) includes secrets as a special
type. You can access secrets only with the <Fn>secret</Fn>
function.
...
<Op>python</Op> operator. You must not modify a secret
...
Tenzir hides the `tenzir.secrets` section from the <Fn>config</Fn>
function.
