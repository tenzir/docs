...
1. [**Pipeline**](pipelines): A sequence of operators responsible for loading,
   parsing, transforming, and routing data. Pipelines are the core mechanism for
   data processing.
2. [**Node**](node): A running process that manages and executes pipelines.
3. [**Platform**](platform): A higher-level management layer that provides
...
on premises](../installation/deploy-the-platform.md) in fully air-gapped
environments.