...
![Pipeline Chaining](pipeline/operator-chaining.svg)
...
![Pipeline Structure](pipeline/operator-types.svg)
...
![Upstream and Downstream Types](pipeline/operator-table.svg)
...
[`serve`](/references/operators/serve) to make the pipeline a REST API
...
[`buffer`](/references/operators/buffer) accepts both bytes and events.

Many Tenzir pipelines use the [`from`](/references/operators/from) and
[`to`](/references/operators/to) operators to get data in and out,
...
![Operator Composition Example 1](pipeline/operator-composition-example-1.svg)

The operator [`from`](/references/operators/from) is a void-to-events
input operator, [`where`](/references/operators/where) an
events-to-events transformation operator, and
[`to`](/references/operators/to) an events-to-void output operator.
...
![Operator Composition Example 2](pipeline/operator-composition-example-2.svg)
...
![Operator Composition Examples](pipeline/operator-composition-variations.svg)
...
[`sort`](/references/operators/sort) operator is an example of such an
...
![Multi-schema Example](pipeline/multi-schema-example.svg)

Some operators only work with exactly one instance per schema internally, such
as [`write_csv`](/references/operators/write_csv), which first writes a
...
![Unified Processing](pipeline/unified-processing.svg)
...
1. **Ingest**: to store data at a node, create a pipeline that ends with
   [`import`](/references/operators/import).
2. **Query**: to run a historical query over data at the node, create a pipeline
   that begins with [`export`](/references/operators/export).
...
![Database Architecture](pipeline/storage-engine-architecture.svg)
...
![Implicit vs. Explicit](pipeline/implicit-vs-explicit-networking.svg)
...
import
```
Or one that begins with [`export`](/references/operators/export):
...
![Implicit Networking](pipeline/implicit-networking.svg)
...
operator location with the [`local`](/references/operators/local) and
[`remote`](/references/operators/remote) operators.
...
![Pipeline Fabric](pipeline/pipeline-fabric.excalidraw.svg)
