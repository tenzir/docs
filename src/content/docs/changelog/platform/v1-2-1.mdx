---
title: Tenzir Platform v1.2.1
sidebar:
  badge:
    text: v1.2.1
    variant: note
---

This patch release contains the following changes, bug fixes and improvements
over [Tenzir Platform
v1.2](https://github.com/tenzir/platform/releases/tag/v1.2.0):

* This release handles OIDC issuer URLs without a trailing slash correctly.
* This release fixes an issue where the `PUBLIC_OIDC_SCOPES` variable is
  ignored.
