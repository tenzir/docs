...
Tenzir can read from and write to local files, cloud object storage, standard
...
Use <Op>from_file</Op> to read files with glob patterns, automatic format
detection, and file monitoring. For writing, use <Op>to_file</Op> with a print
operator.
...
Use the dedicated Unix domain socket operators for local socket streams. When
Tenzir connects to an existing socket, use <Op>from_unix_socket</Op> or
...
When Tenzir owns the listening socket path, use <Op>accept_unix_socket</Op>:
