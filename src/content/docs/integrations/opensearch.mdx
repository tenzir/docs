...
To send data to the OpenSearch via [Logstash][ship-via-logstash], use the
standard Logstash configuration with the OpenSearch output plugin. Here's an
example of your `pipeline.conf`:

[ship-via-logstash]: https://opensearch.org/docs/latest/tools/logstash/ship-to-opensearch/
