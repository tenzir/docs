...
[`from_fluent_bit`](/references/operators/from_fluent_bit) and
[outputs][outputs] via
...
translates to Tenzir's [`from_fluent_bit`](/references/operators/from_fluent_bit)
operator as follows:
...
with the [`to_fluent_bit`](/references/operators/to_fluent_bit) operator working
exactly analogous.
...
:::tip
Use the dedicated [`to_splunk` operator](/references/operators/to_splunk)
