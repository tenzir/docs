...
[Fluent Bit](https://fluentbit.io) is an open source observability pipeline.
Tenzir embeds Fluent Bit, exposing all its [inputs][inputs] via
<Op>from_fluent_bit</Op> and [outputs][outputs] via <Op>to_fluent_bit</Op>
...
translates to Tenzir's <Op>from_fluent_bit</Op>
operator as follows:
...
with the <Op>to_fluent_bit</Op> operator working
exactly analogous.
...
:::tip
Use the dedicated <Op>to_splunk</Op> operator
to send events to a Splunk HEC.
:::

### Collect host metrics

Use Fluent Bit's Node Exporter Metrics input plugin to collect host metrics from
Linux systems:
