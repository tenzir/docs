...
endpoint, use the <Op>to_splunk</Op> operator.

For example, deploy the following pipeline to forward
<Integration>suricata</Integration> alerts to Splunk:
...
<Op>from_fluent_bit</Op> source operator.
