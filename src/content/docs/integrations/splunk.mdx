...
endpoint, use the [`to_splunk`](/references/operators/to_splunk) operator.

For example, deploy the following pipeline to forward
[Suricata](/integrations/suricata) alerts to Splunk:
...
[`to_splunk`](/references/operators/to_splunk) operator.
...
[`from_fluent_bit`](/references/operators/from_fluent_bit) source operator.
