...
| ---------------- | ---------- | --------------- | ------------------ | ------ |
| `delta`          | Per-commit | Managed Delta   | Full Unity Catalog | ✅     |
| `iceberg`        | Zero       | Managed Iceberg | Full Unity Catalog | ⏳     |
| `external-delta` | Zero       | External Delta  | External table     | ⏳     |
...
### External Delta (`external-delta`) — _Planned_
...
| Factor                       | Managed Delta | Managed Iceberg | External Delta |
| ---------------------------- | :-----------: | :-------------: | :------------: |
| Zero DBU ingestion           |      ❌       |       ✅        |       ✅       |
...
service principal. Consider using a [secret store](/explanations/secrets) to
...
- **One file per partition**: Queries filtering on partition columns skip irrelevant files entirely.
...
to_databricks
  ...
  client_secret=$client_secret,
  warehouse_id="abc123def456",
  partition_by=[time.round(1d), class_uid],
