...
a pipeline with HTTP responses. `from_http` streams response body chunks into
its parser sub-pipeline as they arrive. Both operators automatically infer the
response format from the URL extension or `Content-Type` header.
...
Use [`to_http`](/reference/operators/to_http) to send events to a webhook or
API endpoint as a single HTTP request per operator invocation. A printer
sub-pipeline turns the input events into the request body bytes, which Tenzir
streams directly into the outgoing request. This is useful for pushing alerts
to webhooks, forwarding events to SIEMs, or sending periodic batches to
external APIs.
