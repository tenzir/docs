...
[`load_http`](/references/operators/load_http) and
[`save_http`](/references/operators/save_http) for seamless URL-style use
via [`from`](/references/operators/from) and
...
[`read_json`](/references/operators/read_json) and
[`write_ndjson`](/references/operators/write_ndjson).
:::
