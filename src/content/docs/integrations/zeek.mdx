...
Shipping](https://tenzir.com/blog/native-zeek-log-rotation-and-shipping)
provides further details on this method.
...
The list of arguments to `zeek-cut` are the column names of the log.
The <Op>select</Op> operator performs the equivalent in Tenzir after we parse the
logs as Zeek TSV:
...
You can render any data as Zeek TSV log using <Op>write_zeek_tsv</Op>:
