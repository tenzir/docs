...
Zscaler's [Nanolog Streaming Service (NSS)][nss] streams Zscaler logs to
external systems. You can use Zscaler's Cloud NSS or deploy an on-prem NSS
server, and Tenzir can receive logs in either case.
...
Spin up a Tenzir pipeline that accepts <Integration>tcp</Integration> connection
and parses <Integration>syslog</Integration>:
