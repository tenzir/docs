...
[`from`](/references/operators/from) and then spawns a nested pipeline per
accepted connection, each of which reads a stream of GELF messages using
[`read_gelf`](/references/operators/read_gelf). Graylog will connect to
...
[`publish`](/references/operators/publish):
