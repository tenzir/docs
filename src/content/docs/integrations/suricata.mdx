...
[Suricata](https://suricata.io/) is a network monitor with a rule matching
engine to detect threats. Use Tenzir to acquire, process, and store Suricata
logs.
...
Onboard Suricata EVE JSON logs via the
<Op>read_suricata</Op> operator as follows:
...
This is different from Suricata's `unix-command` socket. The command socket is
for JSON control commands, for example through `suricatasc`; it doesn't carry
EVE events.
