...
<Guide>ai-workbench/use-agent-skills#use-the-asim-skill</Guide> to use the
`tenzir-asim` skill. The skill helps choose ASIM schemas, inspect fields,
resolve aliases, and map source telemetry with canonical field names such as
`EventSchema`, `SrcIpAddr`, and `DstIpAddr`.
