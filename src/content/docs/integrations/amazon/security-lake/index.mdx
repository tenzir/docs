...
1. Provide a globally unique 20-character source name using the pattern
   `tnz-${class_uid}` (for example, `tnz-ocsf-1001` for File System Activity)
...
which provides Python scripts to automate the setup process. The easiest way to
use these tools is with `uvx` (from the `uv` Python package manager):

```bash
# Create a custom source for Network Activity (class_uid 4001)
uvx security-lake-tools create-source \
  --name "tnz-ocsf-4001" \
  --event-class "Network Activity" \
  --account-id "123456789012" \
  --region "eu-west-2"

# List all available OCSF event classes
uvx security-lake-tools list-event-classes
```

The tools automate:

- Custom source creation with proper naming (`tnz-${class_uid}` pattern)
