...
Tenzir can send events to ASL via the [`to_asl`
operator](/references/operators/to_asl).

## Configuration

Follow the [standard configuration instructions](..) to authenticate with your
AWS credentials.

Set up a custom source in ASL and use its S3 bucket URI with the `to_asl`
operator.
