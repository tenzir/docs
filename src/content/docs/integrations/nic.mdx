...
The [`load_nic`](/references/operators/load_nic) produces a stream of bytes
in PCAP file format:
...
with [`load_file`](/references/operators/load_file) and It Just Works™.
...
[`nics`](/references/operators/nics) operator to identify suitable candidates:
...
[`decapsulate`](/references/functions/decapsulate) function to decode the
binary `data`:
...
[`community_id`](/references/functions/community_id) function to compute this
