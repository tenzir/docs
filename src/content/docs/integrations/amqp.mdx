...
When receiving messages, Tenzir emits each AMQP payload in the `message` field
as a blob. Parse this field in the pipeline when the payload contains structured
data.
...
By default, `to_amqp` sends each input event as NDJSON. Use the `message`
parameter to send a specific string or blob field instead.
