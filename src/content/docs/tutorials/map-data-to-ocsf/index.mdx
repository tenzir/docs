...
// (5) Minimize nulls in the remaining unmapped data.
drop_null_fields unmapped

// (6) Populate sibling fields and validate the final OCSF shape.
ocsf::derive
ocsf::cast
```
...
3. The assignment `this = ocsf` hoists the mapped event into the top-level
...
5. We drop null fields from `unmapped` to keep the output clean.
6. We keep the mapping terse by only mapping the `*_[u]id` field and relying
...
ocsf.app_name = move ocsf.unmapped.service
```
...
ocsf.traffic = {
  bytes_in: ocsf.unmapped.resp_bytes,
  bytes_out: ocsf.unmapped.orig_bytes,
  packets_in: ocsf.unmapped.resp_pkts,
  packets_out: ocsf.unmapped.orig_pkts,
  bytes: ocsf.unmapped.orig_bytes + ocsf.unmapped.resp_bytes,
  packets: ocsf.unmapped.orig_pkts + ocsf.unmapped.resp_pkts,
}
drop ocsf.unmapped.resp_bytes
drop ocsf.unmapped.orig_bytes
drop ocsf.unmapped.resp_pkts
drop ocsf.unmapped.orig_pkts
```

Here's what happens here:

- The expression `$proto_nums[ocsf.unmapped.proto]` takes the value of Zeek's
  `proto` field and uses it as an index into a static record `$proto_nums`. Add
  a `?` at the end to avoid warnings when the lookup returns `null`, and use
  the inline `else` expression for the fallback value.
...
final event, trims null residue, and runs shared OCSF helpers such as
<Op>ocsf::derive</Op> and <Op>ocsf::cast</Op>. This mirrors how larger package
mappers handle shared fields and event-specific mappings.
