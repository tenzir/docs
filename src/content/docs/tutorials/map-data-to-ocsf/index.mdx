...
1. With `this = { zeek: this }`, we keep the original event in a
   source-specific working namespace. This approach avoids name clashes when we
   create new OCSF fields in the next steps.
2. The main work takes place here. Our approach is structured: for every field
   ...
   `ocsf.x = move zeek.y`. If a field needs to be used multiple times
...
4. The assignment `this = {...ocsf, unmapped: zeek}` returns the mapped OCSF
   event and stores the remaining source fields as `unmapped` for review.
...
as you go, return the residue as `unmapped`, and validate the final shape with
<Op>ocsf::derive</Op> and <Op>ocsf::cast</Op>. This approach ensures you don't
