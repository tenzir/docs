...
5. We keep the mapping terse by only mapping the `*_[u]id` field and relying on
   [`ocsf::derive`](/reference/operators/ocsf/derive) to auto-populate the
   corresponding sibling fields.
...
rely on [`ocsf::derive`](/reference/operators/ocsf/derive) to populate sibling
...
our extensive [guide on parsing log messages from
strings](/guides/parsing/parse-log-messages-from-strings) for more details.
