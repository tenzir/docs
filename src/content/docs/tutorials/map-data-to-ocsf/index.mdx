...
// (3) Return the mapped OCSF event.
this = ocsf
...
4. The assignment `this = ocsf` returns the mapped event from the working
...
The mapper keeps intermediate results under `ocsf` internally, returns the final
OCSF event, and leaves callers to run shared OCSF helpers such as
<Op>ocsf::derive</Op> and <Op>ocsf::cast</Op>. This mirrors how larger package
mappers handle shared fields, fallback Base Event mapping, and event-specific
mappings.
