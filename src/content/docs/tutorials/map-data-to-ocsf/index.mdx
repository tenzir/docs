...
OCSF is a vendor-agnostic event schema, also known as a taxonomy, that defines
structure and semantics for security events. Here are some key terms you need
to know to map events:

- **Attribute**: a unique identifier for a specific type, such as `parent_folder`
...
- **Event Class**: a description of an event that uses specific attributes,
  such as `HTTP Activity` and `Detection Finding`.
- **Category**: a group of event classes, such as `System Activity` or
  `Findings`.
...
individual fields. The <Op>read_zeek_tsv</Op> operator parses the above
...
Activity](https://schema.ocsf.io/1.8.0/classes/network_activity). We use OCSF
v1.8.0 throughout this tutorial.
...
// Keep the source event as mapping residue inside the OCSF record.
ocsf = {}
ocsf.unmapped = this
...
ocsf.time = move ocsf.unmapped.ts // 👈 remove source field while mapping
...
ocsf.metadata = {
  ...
  original_event_uid: move ocsf.unmapped.uid,
  version: "1.8.0",
}
...
drop ocsf.unmapped.id // 👈 remove source field after mapping
...
// (3) Hoist all `ocsf` attributes into the root.
this = ocsf

// (4) Populate sibling fields and validate the final OCSF shape.
ocsf::derive
ocsf::cast
```
...
1. With `ocsf = {}` and `ocsf.unmapped = this`, we create a dedicated OCSF
   record and keep the original event as mapping residue. This approach avoids
   name clashes when we create new fields in the next steps. Drop
   `ocsf.unmapped.ocsf?` to avoid carrying a nested OCSF record from upstream.
2. The main work takes place here. Our approach is structured: for every field
   ...
   keyword to perform (1) and (2) together, for example,
   `ocsf.x = move ocsf.unmapped.y`. If a field needs to be used multiple times
   in the same expression, use
   the <Op>drop</Op> afterwards.
3. Assign `@name` to the target OCSF event class so that downstream pipelines
   can filter by schema.
4. The assignment `this = ocsf` hoists the mapped event into the top-level
   record. Because mapped source fields were removed from `ocsf.unmapped`, the
   remaining `unmapped` record contains everything that still needs review.
5. We keep the mapping terse by only mapping the `*_[u]id` field and relying
   on <Op>ocsf::derive</Op> to auto-populate the corresponding sibling fields.
   Then <Op>ocsf::cast</Op> validates the final event against the OCSF schema.
...
rely on <Op>ocsf::derive</Op> to populate sibling fields—an optimization to
write more concise mappings. This would simplify the above snippet to:
...
The computed sibling fields for `<field>_id` often have the pattern
`<field>_name` or simply `<field>`.
...
When Zeek records a connection duration, using `+` with a value of type `time`
and `duration` yields a new `time` value, just as you'd expect.
...
drop ocsf.unmapped._path? // implied in metadata.log_name
ocsf.app_protocol_name = move ocsf.unmapped.service
```

We use `?` when accessing fields that are not always present, such as
`ocsf.unmapped._write_ts?`. If you omit the `?`, the pipeline emits a warning
...
if ocsf.unmapped.geo.orig.country_code? != null or ocsf.unmapped.geo.orig.region? != null or ocsf.unmapped.geo.orig.city? != null or ocsf.unmapped.geo.orig.latitude? != null or ocsf.unmapped.geo.orig.longitude? != null {
  ocsf.src_endpoint.location = {}
  if ocsf.unmapped.geo.orig.country_code? != null {
    ocsf.src_endpoint.location.country = move ocsf.unmapped.geo.orig.country_code
  }
  if ocsf.unmapped.geo.orig.region? != null {
    ocsf.src_endpoint.location.region = move ocsf.unmapped.geo.orig.region
  }
  if ocsf.unmapped.geo.orig.city? != null {
    ocsf.src_endpoint.location.city = move ocsf.unmapped.geo.orig.city
  }
  if ocsf.unmapped.geo.orig.latitude? != null {
    ocsf.src_endpoint.location.lat = move ocsf.unmapped.geo.orig.latitude
  }
  if ocsf.unmapped.geo.orig.longitude? != null {
    ocsf.src_endpoint.location.long = move ocsf.unmapped.geo.orig.longitude
  }
}
if ocsf.unmapped.geo.resp.country_code? != null or ocsf.unmapped.geo.resp.region? != null or ocsf.unmapped.geo.resp.city? != null or ocsf.unmapped.geo.resp.latitude? != null or ocsf.unmapped.geo.resp.longitude? != null {
  ocsf.dst_endpoint.location = {}
  if ocsf.unmapped.geo.resp.country_code? != null {
    ocsf.dst_endpoint.location.country = move ocsf.unmapped.geo.resp.country_code
  }
  if ocsf.unmapped.geo.resp.region? != null {
    ocsf.dst_endpoint.location.region = move ocsf.unmapped.geo.resp.region
  }
  if ocsf.unmapped.geo.resp.city? != null {
    ocsf.dst_endpoint.location.city = move ocsf.unmapped.geo.resp.city
  }
  if ocsf.unmapped.geo.resp.latitude? != null {
    ocsf.dst_endpoint.location.lat = move ocsf.unmapped.geo.resp.latitude
  }
  if ocsf.unmapped.geo.resp.longitude? != null {
    ocsf.dst_endpoint.location.long = move ocsf.unmapped.geo.resp.longitude
  }
}
...
// `ocsf.unmapped.conn_state` to extract success/failure, this would go beyond
// the tutorial.
ocsf.status_id = 99
ocsf.status = "Other"
...
- The expression `$proto_nums[ocsf.connection_info.protocol_name]` takes the
  moved Zeek `proto` value and uses it as an index into a static record
...
- Set `is_src_dst_assignment_known` because Zeek's `id.orig_*` fields identify
  the connection initiator and `id.resp_*` fields identify the responder.
- Move Layer 2 addresses and GeoIP enrichment into endpoint attributes instead
  of leaving them in `unmapped`.
- Use <Fn>is_v6</Fn> on the connection IPs to identify IPv6 connections.
...
built-in <Op>read_zeek_tsv</Op> operator made this trivial. But it often requires a lot
more elbow grease to get there. Check out our extensive [guide on parsing string
fields](/guides/parsing/parse-string-fields) for more details. We haven't yet
addressed the other end of the pipeline: data _offboarding_. Our examples run
...
(re)usable. To turn it into a package capability, we split the pipeline into
independently usable snippets. The next section describes how to achieve this.

### Step 3: Package the mapping

To make our OCSF mapping more reusable, we extract it as a **user-defined
...
need to know right now is that packages are one-click installable bundles of
operators, examples, tests, and deployable pipelines. After installation, you
can call the newly introduced mapping operators from any pipeline.
...
from_stdin
read_zeek_tsv
zeek::ocsf::map // 👈 Populates the `ocsf` record with a package UDO.
this = ocsf
ocsf::derive
ocsf::cast
to_stdout
...
combination of other operators: one dispatcher, shared setup in the `ocsf`
record, and event-specific operators for each log type.
...
The mapper keeps intermediate results under `ocsf` until the caller hoists the
final event and runs shared OCSF helpers such as <Op>ocsf::derive</Op> and
<Op>ocsf::cast</Op>. This mirrors how larger package mappers handle shared
fields, fallback Base Event mapping, and event-specific mappings.
...
how to test your OCSF mappings.
:::
...
Now there's a \*.txt file next to the test scenario. Verify that it has the
...
After running `uvx tenzir-test` again without any flags, you get the following
output:
...
Perfect. Now proceed with all log types and you have a production-grade
package.

### Step 4: Install and use the package

After you have fleshed out the complete package, [install
it](/guides/packages/install-a-package), either interactively
via <Op>package::add</Op>, or IaC-style by putting it into a git repo and pointing
...
This tutorial showed you how to map security data to OCSF using TQL pipelines.
You learned:
...
residue in `ocsf.unmapped`, map attributes group by group while removing source
fields as you go, and validate the final shape with <Op>ocsf::derive</Op> and
<Op>ocsf::cast</Op>. This approach ensures you don't miss any fields and makes
your mappings easy to maintain and extend.
