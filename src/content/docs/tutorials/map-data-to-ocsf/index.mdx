...
OCSF is a vendor-agnostic event schema (aka. "taxonomy") that defines structure
and semantics for security events. Here are some key terms you need to know to
map events:
...
2. The main work takes place here. Our approach is structured: for every field
   ...
   keyword to perform (1) and (2) together, e.g., `ocsf.x = move source.y`. If a
   field needs to be used multiple times in the same expression, use
...
5. We keep the mapping terse by only mapping the `*_[u]id` field and relying on <Op>ocsf::derive</Op> to auto-populate the corresponding sibling fields.
...
The computed sibbling fields for `<field>_id` often have the pattern
...
<details> <summary>Generated *.txt test scenario baseline</summary>
