...
For maximum flexibility, we should split the pipeline into multiple,
independently usable pieces. The next section describes how to achieve this.

### Step 3: Package the mapping

To make our OCSF mapping reusable, we put it into a
[package](/explanations/packages) as a **user-defined operator**. There is an
...
Let's assume your package comes with a user-defined operator called
...
Notice how `zeek::ocsf::map` has two modules that are double-colon-separated:
`zeek` and `ocsf`. The directory structure in the package determines the module
hierarchy:
...
In this layout, you'd put the mapping pipelines in the following directories:
...
As you add more meat on the bones of the `zeek` package, you could provide a
variety of different onboarding pipelines to facilitate getting data in. Refer
to our [package writing tutorial](/tutorials/write-a-package) for more details.

### Step 4: Install and use the package

After you have fleshed out the complete package, [install
it](/guides/basic-usage/install-a-package) via
[`package::add`](/reference/operators/package/add) or IaC-style by putting it
into a git repo and point your Tenzir instance to it via the `--package-dirs`
command line flag or `tenzir.package-dirs` configuration option.
