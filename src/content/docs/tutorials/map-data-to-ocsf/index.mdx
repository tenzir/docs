...
individual fields. The <Op>read_zeek_tsv</Op>
...
ocsf.time = take zeek.ts // 👈 remove source field while mapping
ocsf.duration = take zeek.duration
...
2. The main work takes place here. Our approach is structured: for every field
   in the source event, (1) map it, and (2) remove it. Ideally, use the `take`
   keyword to perform (1) and (2) together, e.g., `ocsf.x = take source.y`. If
   a field needs to be used multiple times in the same expression, use the
   <Op>drop</Op> afterwards.
...
5. We keep the mapping terse by only mapping the `*_[u]id` field and relying on
   <Op>ocsf::derive</Op> to auto-populate the corresponding sibling fields.
...
rely on <Op>ocsf::derive</Op> to populate sibling
fields—an optimization to write more concise mappings. This would simplify the
above snippet to:
...
ocsf.metadata = {
  log_name: "conn.log",
  logged_time: take zeek._write_ts?,
  product: {
    name: "Zeek",
    vendor_name: "Zeek",
...
ocsf.app_name = take zeek.service
```
...
ocsf.connection_info = {
  community_uid: take zeek.community_id?,
  protocol_name: take zeek.proto,
...
ocsf.status_id = 99
ocsf.status = "Other"
ocsf.status_code = take zeek.conn_state
ocsf.traffic = {
...
<Op>read_zeek_tsv</Op> operator made this
trivial. But it often requires a lot more elbow grease to get there. Check out
our extensive [guide on parsing string fields](/guides/parsing/parse-string-fields)
for more details.
...
it](/guides/packages/install-a-package), either interactively via
<Op>package::add</Op>, or IaC-style by putting it
...
The key to successful OCSF mapping is systematic organization: take your source
