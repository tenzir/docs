...
[`read_zeek_tsv`](/references/operators/read_zeek_tsv) operator:
...
- To check whether we have an IPv4 or an IPv6 connection, we call
  ...
  numerous [functions](/references/functions) that make mapping a breeze.
...
[`publish`](/references/operators/publish) and
[`subscribe`](/references/operators/subscribe) operators send events to, and
