...
- We use `.?` for the field access `zeek.?_write_ts` because not all Zeek logs
  have the `_write_ts` field. If we just used `.` then the absence of
...
- The expression `$proto_nums.get(zeek.proto, -1)` takes the value of Zeek's
  `proto` field (e.g., `tcp`) and uses it as index into a static record
  `$proto_nums`. The `get` function has a second argument that represents the
  value in case the lookup fails. Use `get` in this way when you want to avoid
  generating warnings when a field does not exist.
...
drop zeek.proto, zeek.local_orig, zeek.local_resp
ocsf.status = "Other"
move ocsf.status_code = zeek.conn_state
ocsf.status_id = 99
ocsf.traffic = {
