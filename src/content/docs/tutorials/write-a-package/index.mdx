...
We'll begin by managing the lookup table. But first we need to get the package
...
Use top-level `categories` for the Tenzir Library grouping. Valid values are
`sources`, `destinations`, `mappings`, and `contexts`. Tenzir accepts the
top-level `metadata` field for external tooling and keeps its contents opaque.
Unknown top-level keys outside the package schema still fail validation, so put
non-engine data under `metadata`.
...
The relative path in the package defines the operator name. After installing
...
objects](https://schema.ocsf.io/1.6.0/objects/osint), the standardized
representation of indicators of compromise (IOCs) in OCSF.
...
This additional structure is normal when upgrading raw data to a semantically
richer representation like OCSF. You can always trim the feed down again later,
either automatically with our
<Op>ocsf::trim</Op> operator or manually by
<Op>drop</Op>ping fields. But while the data
...
So far, our operators are static—they always do the same thing. You can make
...
a configurable confidence level and source name. See <Guide>packages/add-operators</Guide> for the full parameter reference,
...
must explicitly enable, add `disabled: true` to the frontmatter. See <Guide>packages/add-pipelines</Guide> for all frontmatter options.
...
using the <Op>every</Op> operator:
...
(<Op>every</Op>) and an output operator
(<Op>publish</Op>). The pipeline produces a new OCSF
...
This pipeline hones in on OCSF Network Activity events (`category_uid == 4`)
that come with a SHA1 TLS certificate fingerprint (`algorithm_id == 2`). If we
have a match, we add the `osint` profile to the event and publish it to a
...
it during installation. See <Guide>packages/configure-inputs</Guide>
...
Since our package ships with user-defined operators, we recommend writing tests
for them:
...
As expected, the test produces a valid OCSF OSINT object. Let's confirm this as
our new baseline:
...
The suite updates the lookup table and verifies the expected values. See <Guide>packages/test-packages</Guide> for suite configuration and <Guide>packages/add-contexts</Guide> for context-specific testing patterns.
...
The interactive wizard guides you through the process. See <Guide>packages/maintain-a-changelog</Guide> for the full release workflow.
