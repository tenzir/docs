...
So far, our operators are static—they always do the same thing. But what if we
want to make them more flexible? [Parameterized
operators](/explanations/packages#parameterized-operators) allow you to pass
positional and named arguments, just like built-in operators.
...
Override confidence by passing `confidence` as named argument:

```tql
from {hash: "abc123"}
sslbl::tag_threat hash, confidence="High"
```
...
Override both by passing `confidence` and `source`:

```tql
from {hash: "abc123"}
sslbl::tag_threat hash, confidence="High", source="Custom Feed"
```
...
The `field` type is special—it accepts field selectors like `hash` and
...
| -------- | ------------------------------------ | ------------- |
| `field`  | A field selector                     | `field_name`  |
...
Omit the `type` field to accept any value. All types accept `null`. Type
checking only applies to constant expressions; for runtime data, the type serves
...
from {hash: "test123"}
sslbl::tag_threat hash, confidence="High", source="Test"
```
...
:::tip[Operators vs. Pipelines]
With our user-defined operators, we get building blocks, but not yet entire
pipelines. Packages that only ship operators provide capabilities—they offer
reusable components that other pipelines can compose. This makes them versatile:
users decide when and how to use the operators.

Packages that include pipelines execute immediately upon installation. This is
useful for background tasks like periodic data fetching, but it means the
package takes action without explicit user invocation.
...
installation](/guides/packages/install-a-package), e.g., when using
...
the Tenzir ecosystem, it is actually fun. 🕺
...
[`tenzir-ship`](/reference/ship-framework) tool. From the package
...
```sh
uvx tenzir-ship show              # show all entries in table format
uvx tenzir-ship show unreleased   # show just unreleased entries
uvx tenzir-ship show --markdown   # render all entries as Markdown
uvx tenzir-ship show --json       # render all entries as JSON
uvx tenzir-ship show --card 1     # render a specific entry as card
```

For more details on how to maintain a changelog and create releases, see our
[guide on maintaining a
changelog](/guides/packages/maintain-a-changelog).
