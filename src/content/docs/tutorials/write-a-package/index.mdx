...
Check out our [dedicated tutorial on mapping data to
OCSF](/tutorials/map-data-to-ocsf). You'll learn how to use packages to scale
the process of creating mappings.
:::
...
We'll put a trimmed version of https://sslbl.abuse.ch/blacklist/sslblacklist.csv
into `tests/inputs/`:
...
We can watch the output by running the test harness in passthrough mode
(`--passthrough` or `-p`):
...
i   1× tenzir (v5.16.0+g741888f304)
...
This looks good! Let's confirm the baseline/reference via `--update`::
...
This created a [`fetch.txt`](/packages/sslbl/tests/fetch.txt) file next to the
[`fetch.tql`](/packages/sslbl/tests/fetch.tql) file. Future runs will use it for
comparisons.
...
Again, we first watch it first in passthrough mode:
