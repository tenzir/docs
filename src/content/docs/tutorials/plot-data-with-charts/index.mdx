...
1. **Pie**: <Op>chart_pie</Op>
2. **Bar**: <Op>chart_bar</Op>
3. **Line**: <Op>chart_line</Op>
4. **Area**: <Op>chart_area</Op>
...
1. [Run a pipeline](/guides/basic-usage/run-pipelines/) to prepare the
   data.
...
![Pipeline to Chart](./pipeline-to-chart.svg)
...
1. Click the **Dashboard** button.
   ...
   ![Add Chart to Dashboard](./add-chart-to-dashboard-1.png)

2. Enter a title for the chart, then click **Add to Dashboard**.

   ![Add Chart Title](./add-chart-to-dashboard-2.png)

3. View the chart in your dashboard.

   ![Chart in Dashboard](./add-chart-to-dashboard-3.png)
...
1. Click the download button in the top-right corner.
   ...
   ![Download Chart](./download-chart-explorer.png)
...
2. Click **Download**
   ...
   ![Download Chart](./download-chart-dashboard.png)
...
2. **Plot the data**: Add the <Op>chart_bar</Op>
...
![Grouped bar chart](./chart-bar-grouped.png)
...
![Stacked bar chart](./chart-bar-stacked.png)
...
![Unscaled bar chart](./chart-bar-nolog.png)
...
![Log-scaled bar chart](./chart-bar-log.png)

Now, you can clearly see all the values!

:::caution[Interpreting Log-Scaled Plots]
...
1. **Shape your data**: For our line chart demo, we'll use some internal node
   metrics provided by the <Op>metrics</Op> operator.
...
2. **Plot the data**: Add the <Op>chart_line</Op> operator to visualize
...
3. **Aggregate to reduce the resolution**: Plotting metrics with a 1-second
   ...
   ![Line chart with summarize](./chart-line-summarize.png)
...
![Line chart with multiple series](./chart-line-multiple.png)
...
<Op>chart_line</Op> with
<Op>chart_area</Op> and will get a working
plot.
...
values of the series add up, helping you _compare contributions_ from different
...
![Stacked area chart](./chart-area-stacked.png)

Notice the difference in the y-axis interpretation:

- Without stacking, the areas _overlap_ each other.
- With stacking, the areas become _disjoint_ and _cumulatively add up_ to the
  total height.

## Optimize Plotting with Inline Expressions

While it's intuitive to _first_ prepare your data and _then_ start thinking
...
![Line chart with resolution](./chart-line-resolution.png)
...
- Specifying a duration with `resolution` option creates nicer buckets than
  naive rounding, as it uses a dynamic floor for the minimum and a ceiling for
