...
This tutorial teaches you to write TQL that is clear, efficient, and
maintainable. It assumes you already know basic TQL syntax and operators, and
shows you how experienced TQL developers approach common patterns.
...
Idiomatic TQL follows consistent patterns that leverage the language's
strengths:
...
This tutorial shows you these patterns through concrete examples, comparing
idiomatic approaches with common pitfalls.
...
When writing vertical structures, use trailing commas consistently to improve
maintainability.
...
TQL's `if` and `where` don't require parentheses around conditions. Adding them
creates visual noise without any benefit.
...
when a field takes exactly one of two values based on a single boolean.
...
When the source field is only needed as the condition and not used elsewhere,
wrap a `move` in parentheses to consume and test it in a single expression — no
...
When a condition checks whether a field matches one of several values, `in` with
a list is cleaner than a chain of `or` comparisons. This works anywhere you
write a condition, such as `where` or `if`.
...
empty strings to represent null, use the <Op>replace</Op> operator to normalize
them instead of writing conditional logic.
...
Don't confuse the <Op>replace</Op> operator with the <Fn>replace</Fn> function:
...
When you aggregate keys inside event-time windows, put <Op>window</Op> outside
<Op>group</Op> if one stream-wide event-time clock should close all buckets. This
keeps sparse keys from holding their own windows open until another event for the
same key arrives, `idle_timeout` fires, or the input ends.
...
Use <Op>group</Op> around <Op>window</Op> only when each key needs an independent
event-time clock. For most stream detections, the outer <Op>window</Op> bounds
...
The <Op>strict</Op> operator escalates all warnings to errors within its scope,
stopping the pipeline when data quality issues occur.
