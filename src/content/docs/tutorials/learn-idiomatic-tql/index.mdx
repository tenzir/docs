...
✅ Idiomatic — clean and self-documenting:
...
The ternary form is appropriate *only for genuinely binary choices*. The moment
you need a third branch, switch to a record constant lookup:

```tql
// Three or more outcomes → record constant
...
Don't confuse the <Op>replace</Op> operator with the
<Fn>replace</Fn> function:
