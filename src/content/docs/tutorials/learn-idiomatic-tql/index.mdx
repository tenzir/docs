...
wrap a `take` in parentheses to consume and test it in a single expression —
no separate `drop` required:

```tql
connection_info = {
  direction_id: 2 if (take raw.initiated) else 1,
  protocol_ver_id: 6 if (take raw.is_ipv6) else 4,
}
...
you need a third branch, switch to if/else chains or preferably a
[record constant lookup](#use-record-constants-for-mappings-instead-of-if-else-chains):
...
When a condition checks whether a field matches one of several values, `in`
with a list is cleaner than a chain of `or` comparisons. This works anywhere
you write a condition, such as `where` or `if`.
...
### Use `take` expressions to prevent field duplication

✅ Clean field transfers with `take`:

```tql
// Taking fields during transformation
normalized.src_ip = take raw.source_address
normalized.dst_port = take raw.destination.port
normalized.severity = take alert.level
```
...
✅ Take fields that are transformed:

```tql
ocsf.activity_id = take http_methods[method]? else 99
```
...
- Use `take` for fields being transformed to prevent duplication
...
<Op>replace</Op> operator to normalize them instead of
...
The <Op>strict</Op> operator escalates all warnings to
...
ocsf.cloud.account.uid = take source.account_id
ocsf.cloud.region = take source.region
```
