...
Use <Op>replace</Op> to convert placeholders to null
...
Avoid using `drop_null_fields` without arguments on the top-level record. Each
unique combination of present fields creates a distinct schema, leading to
_schema fragmentation_ that defeats the purpose of normalization. It also
