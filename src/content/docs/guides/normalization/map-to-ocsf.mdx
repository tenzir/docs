...
- **Use `move`**: Transfer fields with `move` to simultaneously assign and
  remove from source, for example `ocsf.time = move src.time_generated`.
  ...
  - tests/
    ...
    - ngfw/
      - ocsf/
        ...
        - base.txt
        - event/
          - network.stdin Traffic log samples
          - network.tql
          - network.txt
          - dns.stdin
          - dns.tql
...
The [`ocsf::cast`](/reference/operators/ocsf/cast) operator is the primary
schema validation gate. It ensures that your mapping produces schema-compliant
output.

Your mapping is complete once [`ocsf::cast`](/reference/operators/ocsf/cast) no
