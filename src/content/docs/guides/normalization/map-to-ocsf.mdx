...
ocsf.time = take src.time_generated
ocsf.start_time = take src.start
ocsf.duration = take src.elapsed
...
ocsf.metadata = {
  ...
  uid: take src.sessionid,
  version: "1.7.0",
}
ocsf.app_name = take src.app
...
- **Isolate source data**: `this = { src: this }` prevents name clashes and
  makes unmapped field collection automatic.
- **Use `take`**: Transfer fields with `take` to simultaneously assign and
  remove from source, for example `ocsf.time = take src.time_generated`.
- **Only use `drop` for multi-use fields**: When a field appears in multiple
  mappings, drop it after the last use. Prefer `take` and single assignments.
- **Collect unmapped**: `this = {...ocsf, unmapped: src}` gathers any
...
This requires that your test file has a sibling `.stdin` input that the [test
