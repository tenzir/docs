...
The identity provider (IdP) handles authentication for the Tenzir Platform. To
use an external IdP, ensure it supports the OIDC protocol, including the _OIDC
...
any of the provided rules match, the platform considers the user an admin. The
...
Rules](/reference/platform/command-line-interface#configure-access-rules) for
more information about the possible types of rules and their syntax. Use the
`tenzir-platform tools` subcommand of the CLI to generate valid JSON objects
that you can enter here.
