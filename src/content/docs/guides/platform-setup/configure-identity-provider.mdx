...
When clicking on the _Login_ button in the Tenzir UI, you will be redirected
to your chosen identity provider, who creates a signed token certifying your
identity.

You can either use an external identity provider that is already set up inside
your organization, or run an Identity Provider that is bundled with our
...
and EntraID as Identity Providers for the Tenzir Platform.

## Generic Identity Provider

To use an external IdP, ensure it supports the OIDC protocol, including the _OIDC
...
Use the `Add Client` button in the `Clients` menu on the left. You should configure the new
client as follows:
...
In order to use the `tenzir-platform` CLI you need to set up an additional
...
ID token containing a URL to the image file.

The Tenzir Platform will read that claim and use it as the profile picture in
the top right corner of the user interface, or fall back to a default image if the
