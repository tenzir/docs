...
import architectureDiagram from '../../../../assets/tenzir-platform-aws-architecture.png';
...
<Image src={architectureDiagram} alt="Tenzir Platform AWS Architecture" />
...
This deployment will give you a deployment that mirrors our own setup at
[app.tenzir.com](https://app.tenzir.com) and includes publicly-accessible UI and
nodes endpoints that are gated behind an OIDC login. For a fully VPC-private
deployment, please contact us and we're happy to assist with the setup.
:::

:::tip[AWS Marketplace]
This deployment requires a subscription to the Tenzir Platform - Sovereign Edition in the
AWS Marketplace. You'll need to manually copy the container images from the marketplace
to your ECR repositories before deploying the CloudFormation stack.
:::
...
1. **AWS Account**: An AWS account with appropriate permissions to create
   CloudFormation stacks, VPCs, ECS services, Lambda functions, RDS databases,
   and other resources.
2. **Domain in Route53**: A registered domain name with a Route53 hosted zone
   in your AWS account. The deployment automatically discovers the hosted zone
   and creates DNS records for the platform services.
3. **Marketplace Subscription**: Subscribe to the Tenzir Platform - Sovereign Edition in
   the [AWS Marketplace](https://aws.amazon.com/marketplace).
4. **AWS CLI** (optional): Install and configure the [AWS
   CLI](https://aws.amazon.com/cli/) on your local machine if you prefer CLI-based deployment.

## Step 1: Configure your domain

The platform requires a custom domain where it will be hosted. You need a Route53
hosted zone for your domain.

If you don't already have a hosted zone, create one using the AWS Console or CLI.
...
6. Update your domain's nameservers at your domain registrar to point to the
   Route53 nameservers listed in the hosted zone
...
Update your domain's nameservers at your domain registrar to point to the Route53
nameservers returned by the command.
...
Before deploying the CloudFormation stack, you need to have the container images for the three core services (UI, API, and Gateway) available in your AWS account. If you purchased the Tenzir Platform from the AWS Marketplace, follow the steps below to copy the images.
...
Save this script to a file (e.g., `copy-images.sh`), make it executable, and run it:
...
The script copies the three core service container images to your ECR repositories. This process
typically takes 10-15 minutes depending on your network speed.
...
After copying the images, deploy the CloudFormation stack using the publicly hosted
template.
...
aws cloudformation create-stack \
  ...
  --parameters \
    ...
    ParameterKey=ContainerImageUI,ParameterValue=$AWS_ACCOUNT.dkr.ecr.$AWS_REGION.amazonaws.com/tenzir/tenzir-platform-ui:$VERSION \
    ParameterKey=ContainerImageAPI,ParameterValue=$AWS_ACCOUNT.dkr.ecr.$AWS_REGION.amazonaws.com/tenzir/tenzir-platform-api:$VERSION \
    ParameterKey=ContainerImageGateway,ParameterValue=$AWS_ACCOUNT.dkr.ecr.$AWS_REGION.amazonaws.com/tenzir/tenzir-platform-gateway:$VERSION \
  --capabilities CAPABILITY_NAMED_IAM
```

Replace `example.org` with your domain name and adjust the version to match the
images you copied in Step 2.
...
6. Configure the parameters:
   ...
   - **UI Container Image**: Full image URI (e.g., `123456789012.dkr.ecr.us-east-1.amazonaws.com/tenzir/tenzir-platform-ui:v0.1.0`)
   - **API Container Image**: Full image URI (e.g., `123456789012.dkr.ecr.us-east-1.amazonaws.com/tenzir/tenzir-platform-api:v0.1.0`)
   - **Gateway Container Image**: Full image URI (e.g., `123456789012.dkr.ecr.us-east-1.amazonaws.com/tenzir/tenzir-platform-gateway:v0.1.0`)
   ...
   - **Use External OIDC Provider**: Whether to use an external identity
     provider instead of AWS Cognito (`false` to use Cognito)
7. Click **Next** through the remaining screens
8. On the final screen, acknowledge that CloudFormation will create IAM
   resources
...
The template supports the following key parameters:
...
- **Authentication Configuration**:
  ...
  - If using external OIDC, you must also provide:
...
3. If using Cognito, you'll be prompted to change your password on first login
...
Note: S3 buckets must be emptied before deletion. You may need to manually empty
the blobs and sidepath buckets before deleting the stack.
