...
selector, switch to the **Authentications** tab, and click
...
When the operator runs, the node first checks its local `tenzir.auth`
configuration for an entry with that name. If none matches, it asks the
connected platform, resolves the referenced secret through the same encrypted
channel it uses for standalone secrets, and applies the strategy-specific
headers (`Authorization: Bearer …`, `Authorization: Basic …`, a custom header
for API keys, or an OAuth token-fetch loop for client credentials).
