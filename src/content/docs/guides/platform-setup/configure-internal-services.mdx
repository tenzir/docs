...
When the Tenzir UI and nodes run in separate networks, nodes might need to use a
different URL to reach the blob storage service than the one the UI uses.
...
Note that this variable needs to be added to the `platform` container directly,
since it is currently not part of any of the example setups:
...
Note that this variable needs to be added to the `app` container directly,
since it is currently not part of any of the example setups:
...
When this variable is set, the platform bypasses the normal OIDC login flow
...
contained in the external JWT using the `TENZIR_PLATFORM_OIDC_TRUSTED_AUDIENCES`
