...
By default, the Tenzir UI is doing requests directly from the browser to the
Tenzir Gateway to query the status of connected nodes. This can be problematic
in some network topologies, in particular in zero-trust networks where
accessing the Tenzir Gateway requires some additional authentication headers.

It is possible to proxy these requests through the Tenzir UI Backend instead,
making all requests from the Tenzir UI Frontend go to the same domain, at
the cost of some additional latency introduced by an additional proxy hop.
