...
```tql
{avg_price: 191.67, item_count: 3, category: "fruit"}
{avg_price: 150.0, item_count: 2, category: "vegetable"}
```
...
```tql
{avg_duration: 38.0, action_count: 1, user: "bob", action: "login"}
{avg_duration: 89.0, action_count: 1, user: "bob", action: "edit"}
{avg_duration: 48.5, action_count: 2, user: "alice", action: "login"}
{avg_duration: 12.0, action_count: 1, user: "alice", action: "view"}
```
...
summarize p50 = quantile(latency, 0.5), p90 = quantile(latency, 0.9), p95 = quantile(latency, 0.95), endpoint
```

```tql
{p50: 120, p90: 135, p95: 135, endpoint: "/api/users"}
{p50: 245, p90: 280, p95: 280, endpoint: "/api/orders"}
```
...
from {server: "web1", cpu: 45},
     ...
     {server: "web2", cpu: 92},
     {server: "web2", cpu: 88}
summarize avg_cpu = mean(cpu), cpu_stddev = stddev(cpu), cpu_variance = variance(cpu), server
```

```tql
{avg_cpu: 48.33, cpu_stddev: 3.51, cpu_variance: 12.33, server: "web1"}
{avg_cpu: 88.33, cpu_stddev: 3.51, cpu_variance: 12.33, server: "web2"}
```
...
summarize most_common_browser = mode(browser), unique_browsers = distinct(browser), login_count = count_if(action == "login")
```
...
Analyze value distributions with [`value_counts()`](/reference/functions/value_counts)
...
from {category: "A", value: 10},
     ...
     {category: "B", value: 25},
     {category: "C", value: 30}
summarize frequencies = value_counts(category), info_entropy = entropy(category)
```
...
summarize all_actions = collect(action), unique_users = distinct(user), event_count = count()
```

```tql
{
  all_actions: ["login", "view", "edit", "login", "logout"],
...
summarize first_reading = first(reading), last_reading = last(reading), avg_reading = mean(reading), sensor
```

```tql
{first_reading: 72, last_reading: 78, avg_reading: 75.0, sensor: "temp1"}
{first_reading: 68, last_reading: 71, avg_reading: 69.5, sensor: "temp2"}
```
...
summarize all_passed = all(passed), any_failed = any(not passed), total_duration = sum(duration)
```
...
unroll requests
summarize endpoint=requests.endpoint, count=count(), avg_duration=mean(requests.duration)
```

```tql
{endpoint: "/api/orders", count: 2, avg_duration: 122.5}
{endpoint: "/api/users", count: 3, avg_duration: 67.67}
```
...
set cpu_alert = metrics.map(m => m.cpu).any(c => c > 80)
set avg_memory = metrics.map(m => m.memory).mean()
set disk_trend = metrics.last().disk - metrics.first().disk
set health_summary = {
...
summarize request_count = count(), avg_duration = mean(duration), error_rate = mean(status >= 400), unique_endpoints = count_distinct(endpoint), method
```

```tql
{request_count: 1, avg_duration: 67.0, error_rate: 0.0, unique_endpoints: 1, method: "DELETE"}
{request_count: 3, avg_duration: 57.33, error_rate: 0.0, unique_endpoints: 2, method: "GET"}
{request_count: 1, avg_duration: 120.0, error_rate: 0.0, unique_endpoints: 1, method: "POST"}
```
...
4. **Use streaming aggregations**: For large datasets, use the `summarize` operator
5. **Combine aggregations**: Calculate multiple statistics in one pass for efficiency
