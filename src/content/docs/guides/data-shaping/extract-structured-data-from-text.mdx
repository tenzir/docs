...
Many logs use simple key-value formats. The
[`parse_kv()`](/reference/functions/parse_kv) function handles these
automatically:
...
```tql
from {line: "alice|30|engineer|SF"}
set fields = line.parse_xsv(field_separator="|", list_separator=",", null_value="", header=["name", "age", "role", "location"])
```
...
YAML is common in configuration files. Use
...
When data doesn't follow simple patterns,
[`parse_grok()`](/reference/functions/parse_grok) provides powerful pattern
matching:
...
Syslog is ubiquitous in system logging. Use
...
For security tools using CEF, use
...
For IBM QRadar's LEEF format, use
...
set time1 = log1.split(" at ")[1].parse_time("%Y-%m-%d")
