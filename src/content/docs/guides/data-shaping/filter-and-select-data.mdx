...
Use the [`otherwise()`](/reference/functions/otherwise) function to provide default values for null fields:

```tql
from {name: "alice", score: 85},
     {name: "bob"},
     {name: "charlie", score: 95}
set score = score?.otherwise(0)
```
...
4. **Understand null handling**: The [`where`](/reference/operators/where)
   operator skips events where the condition evaluates to null or false.
