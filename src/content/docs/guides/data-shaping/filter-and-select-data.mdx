...
Use the [`set`](/reference/operators/set) operator to add new fields without
...
default value of `0`. However, the access of the `score` field generated a
warning that the field did not exist. If the field existed but had a `null`
value, then there would be no warning.

Use the question mark operator (`?`) to suppress warnings when accessing fields
that may not exist. In that above example, this would be `score? else 0`.
:::
...
select method, path, duration_s = duration_ms / 1000.0
```

```tql
{method: "GET", path: "/api/users", duration_s: 0.045}
{method: "POST", path: "/api/users", duration_s: 0.12}
```
...
3. **Choose the right operator**:
   - Use [`select`](/reference/operators/select) when you want to keep specific fields
   - Use [`drop`](/reference/operators/drop) when you want to remove a few fields from many
   - Use [`set`](/reference/operators/set) when you want to add fields without changing existing ones
