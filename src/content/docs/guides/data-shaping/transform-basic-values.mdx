...
set price = price.int(), quantity = quantity.float()
```
...
set message = status.string() + " - " + (ratio * 100).string() + "%"
```
...
set parsed_time = timestamp.time()
```
...
from {interval: "5s"},
     {interval: "2.5min"}
set parsed_duration = interval.duration()
```
...
from {count: "42", ratio: 3.7},
     {count: "-5", ratio: 2.3}
set count_uint = count.uint(), ratio_uint = ratio.uint()
```
...
set ipv6_check = direct_ip.is_v6()
```
...
set client_ip = client.ip(), network_subnet = network.subnet()
```
...
set is_v4 = ip1.is_v4(), is_v6 = ip3.is_v6(),
    is_private = ip1.is_private(), is_global = ip2.is_global(),
    is_loopback = ip3.is_loopback(), is_multicast = ip2.is_multicast()
```
...
set client_category = client.ip().ip_category(),
...
set link_local1 = addr1.is_link_local(),
...
set name = name.to_title(), code = code.to_upper()
```
...
set input = input.trim(), data = data.trim()
```
...
Use [`secret()`](/reference/functions/secret) to retrieve
...
set temp_c = ((temp_f.float() - 32) * 5 / 9).round(),
