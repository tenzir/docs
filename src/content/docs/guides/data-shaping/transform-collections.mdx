...
Note: The `group_by` function is not available for lists. Use the [`group`](/reference/operators/group) operator in a pipeline for grouping operations.

### Enumerate with indices

Note: `enumerate` is an operator, not a function. Use the [`enumerate`](/reference/operators/enumerate) operator in a pipeline to add index numbers to events.
