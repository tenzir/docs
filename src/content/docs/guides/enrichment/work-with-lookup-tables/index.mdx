...
You can create a lookup table with the <Op>context::create_lookup_table</Op>
operator as a pipeline, or interactively in the platform.

### Create a lookup table from a pipeline

The <Op>context::create_lookup_table</Op> operator creates a new, empty lookup
table:
...
1. In the **Contexts** tab in your node, click the `+` button:
   ...
   ![Create context](./context-add.png)

2. Select type `lookup-table` and enter a name:

   ![Choose name and type](./context-name.png)

3. Click **Create** and observe the new lookup table context:

   ![View created context](./context-created.png)

</Steps>

## Show all available lookup tables

Use the <Op>context::list</Op> operator to retrieve all contexts in your node:
...
Use the <Op>context::remove</Op> operator to delete a lookup table:
...
Use the <Op>context::enrich</Op> operator to enrich events with data from a
lookup table.
...
Use the <Op>context::update</Op> operator to add or update entries. This is
ideal for maintaining dynamic threat intelligence or asset inventory:
...
entries using the <Op>context::update</Op> operator:
...
Use the <Op>context::erase</Op> operator to remove specific entries, useful for
allowlisting, removing false positives, or cleaning up outdated data:
...
Use the <Op>context::inspect</Op> operator to view and analyze the contents of a
lookup table:
...
Periodically poll APIs to maintain fresh reference data, threat intelligence, or
asset information in lookup tables.

### Basic periodic updates

Use the <Op>every</Op> operator to schedule regular API polls that update a
lookup table:
...
Use the <Op>context::save</Op> operator to create backups or migrate lookup
tables between nodes:
...
Use the <Op>context::load</Op> operator to restore lookup tables from backups or
migrate data:
...
:::caution
Reading replaces the entire lookup table state. Existing entries will be lost.
Consider backing up with `context::save` before reading new data.
:::
