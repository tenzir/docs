...
```tql
{num_entries: 0, name: "my_lookup_table"}
```

You can chain this with other operators to immediately populate the table:

```tql
// Create and populate in one pipeline
context::create_lookup_table "threat_indicators"
from_file "indicators.csv" {
  read_csv
}
context::update "threat_indicators", key=domain
```

Check if a context already exists before creating it:

```tql
// List existing contexts and create if not present
context::list
where name != "my_lookup_table"
context::create_lookup_table "my_lookup_table"
```
...
First, create and populate a simple lookup table:
...
Track threats using multiple indicators as compound keys:

```tql
// Create a lookup table for threat intelligence
context::create_lookup_table "threat_intel"

// Populate with multiple indicators per threat
from {
...
extend threat_key = {domain: dest_domain, port: dest_port}
context::enrich "threat_intel", key=threat_key, into=threat_info
```

Implement zone-based access control using compound keys:

```tql
// Create access control lookup table
...
```tql
from_file "network_inventory_staging.bin"
context::load "network_inventory"
```

Load pre-built context from threat intel provider:

```tql
from_http "https://ti-provider.com/contexts/latest-iocs.bin",
  headers={Authorization: f"Bearer {secret('ti_api_key')}"}
context::load "vendor_threat_intel"
```
