...
Deploy a Tenzir node using our CloudFormation template for automated setup:
...
1. Create a new stack using [our
   template](https://s3.amazonaws.com/tenzir-marketplace-resources/single-node-container.yaml):

   ![CloudFormation Create Stack](./aws/cloudformation/01-create-stack.png)

2. Enter the stack details:
...
3. Configure stack options (you can keep the defaults for most deployments):

   ![Stack Options](./aws/cloudformation/03-stack-options.png)

4. Review your configuration and acknowledge the capabilities:

   ![Review Stack](./aws/cloudformation/04-review-stack.png)

5. Click _Submit_ and monitor the progress in the _Events_ tab.

6. Once the stack creation completes, your node will automatically connect to
   your workspace. This may take a few minutes.

</Steps>

:::tip[Stack Management] You can update your node by updating the CloudFormation
stack with new parameters. To remove the node and all associated resources,
...
1. Go to [AWS Marketplace](https://console.aws.amazon.com/marketplace/) and
   ...
   ![AWS Marketplace Tenzir Node](./aws/manual/01-aws-marketplace.png)

2. Accept the terms to subscribe to the offering.

   ![AWS Marketplace Tenzir Node](./aws/manual/02-subscribe.png)

3. Navigate to [Amazon Elastic Container Service (ECS)](https://console.aws.amazon.com/ecs).

   ![Amazon Elastic Container Service (ECS)](./aws/manual/03-ecs.png)
...
4. Create a new cluster. Choose between **EC2** or **Fargate** based on your
   ...
   ![Create a Cluster](./aws/manual/04-create-cluster.png)

5. Create a task definition to specify how the Tenzir node container should run.

   ![Create a Cluster](./aws/manual/05-create-task-definition.png)
...
6. Return to your cluster, navigate to the _Tasks_ tab, and click _Run new
   ...
   ![Create a Cluster](./aws/manual/06-run-task.png)
   ...
   ![Container Overrides](./aws/manual/07-container-overrides.png)
