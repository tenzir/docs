...
Tenzir signs all container images to ensure authenticity and integrity. You can
verify signatures using [Cosign][cosign] or configure [Podman][podman] for
...
Use Cosign with keyless OIDC verification to validate the image was built by
Tenzir's GitHub Actions:
...
Configure Podman for automatic signature verification using the public key from
the [tenzir/tenzir repository][container-signing-pub].

[container-signing-pub]: https://github.com/tenzir/tenzir/blob/main/container-signing.pub
...
Add the following to the `docker` key under `transports` in
...
Copy the public key to the configured path. Podman then verifies signatures
automatically when pulling Tenzir images.

## Linux

For native Linux deployments, first [install
Tenzir](/guides/installation) to get the `tenzir-node` binary.
...
See the [configuration documentation](/explanations/configuration) for details
on how the node loads config files at startup.
...
Replace `v<MAJOR>.<MINOR>.<PATCH>` with the desired version (e.g., `v4.30.3`).
Check our [changelog](/changelog/) for the latest version.

<Tabs syncKey="aws-cloudformation">
<TabItem value="console" label="Console">

1. Navigate to [CloudFormation](https://console.aws.amazon.com/cloudformation/) in the AWS console.
2. Select your stack and click _Update_.
3. Choose _Use current template_ and click _Next_.
...
1. Navigate to [CloudFormation](https://console.aws.amazon.com/cloudformation/) in the AWS console.
2. Select your stack and click _Delete_.
...
3. Create a task definition to specify how the Tenzir node container should run.
   ...
   Each node version has its own image tag. Replace `v<MAJOR>.<MINOR>.<PATCH>`
   with the desired version. Check the [changelog](/changelog/) for
   the latest version.
...
Deploy one or more static nodes on a Kubernetes cluster with the Tenzir Helm
chart. Each entry in the chart's `nodes` list renders as its own StatefulSet
with a persistent volume and a Kubernetes Service. See the
<Explanation>node/helm-chart</Explanation> page for the full option surface.

The chart ships as an OCI artifact at `oci://ghcr.io/tenzir/charts/tenzir-node`.
You need Kubernetes 1.27 or later and Helm 3.8 or later.
...
The chart defaults to `image.tag: latest`, which always pulls the newest
Tenzir release. For production deployments, override the tag with a specific
version so upgrades become explicit, reviewable changes:
...
See the [Tenzir changelog](/changelog/tenzir/) for the list of available
releases. Bumping the tag and re-running `helm upgrade` becomes your
version-bump workflow. To roll back, set the previous tag and `helm upgrade`
again.
...
The chart only restarts nodes whose configuration changed. See
<Explanation>node/helm-chart</Explanation> for how it decides.

### Stop a node

Remove the release. The persistent volumes survive so you can reinstall
...
The chart is signed with [Cosign](https://docs.sigstore.dev/cosign/overview/)
keyless via the GitHub Actions OIDC token, the same scheme used for the
[`tenzir` container image](#verify-container-image-signatures). Verify a
...
cosign verify \
  ...
  --certificate-oidc-issuer="https://token.actions.githubusercontent.com" \
  ghcr.io/tenzir/charts/tenzir-node:v0.1.0
```
