...
Replace `v<MAJOR>.<MINOR>.<PATCH>` with the desired version (e.g., `v4.30.3`). Check the [node changelog](/changelog/node/) for the latest version.
...
3. Create a task definition to specify how the Tenzir node container should run.
   ...
   with the desired version. Check the [node changelog](/changelog/node/) for
   the latest version.
