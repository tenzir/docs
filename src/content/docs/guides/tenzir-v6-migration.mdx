...
| -------------------------------------- | --------------------------- |
| `from_file "/run/source.sock", uds=true` | `from_uds "/run/source.sock"` |
| `to_file "/run/sink.sock", uds=true`     | `to_uds "/run/sink.sock"`     |

Use <Op>from_uds</Op> and <Op>to_uds</Op> when Tenzir connects to an existing
Unix domain socket. Use <Op>accept_uds</Op> when Tenzir owns the listening
socket path for incoming streams.
