...
This guide shows you how to extract structured data from string fields using
TQL's parsing functions. You'll learn to parse JSON, YAML, XML, key-value
pairs, delimited data, timestamps, and log formats like Syslog, CEF, LEEF, and
Windows Event Logs. For custom formats, Grok patterns provide flexible pattern
matching.
...
Many logs use key-value formats. <Fn>parse_kv</Fn>
...
Use <Fn>parse_xsv</Fn> for arbitrary delimiters:
...
Use <Fn>parse_yaml</Fn> for YAML content:
...
Use <Fn>parse_xml</Fn> with an XPath expression to
extract elements from XML strings:
...
Attributes are prefixed with `@` by default. Use `attr_prefix=""` to remove
the prefix.

## Parse Windows Event Logs

Use <Fn>parse_winlog</Fn> for Windows Event Log
XML. It preserves the standard Windows event structure with `System` and
`EventData` sections:
...
Use <Fn>parse_syslog</Fn> for RFC 5424 and
...
Use <Fn>parse_cef</Fn> for Common Event Format
...
Use <Fn>parse_leef</Fn> for Log Event Extended
Format (IBM QRadar):
...
The <Fn>time</Fn> function auto-parses many common
timestamp formats without requiring a format string:
...
It handles ISO 8601 dates, Unix timestamps (with `@` prefix), and relative
expressions like `now` or `5min ago`:
...
For non-standard formats, use <Fn>parse_time</Fn>
...
For complex formats, <Fn>parse_grok</Fn> provides
