...
Use [`parse_xml()`](/reference/functions/parse_xml) with an XPath expression to
...
Use [`parse_winlog()`](/reference/functions/parse_winlog) for Windows Event Log
...
Use [`parse_cef()`](/reference/functions/parse_cef) for Common Event Format
...
Use [`parse_leef()`](/reference/functions/parse_leef) for Log Event Extended
Format (IBM QRadar):
...
The [`time()`](/reference/functions/time) function auto-parses many common
...
For non-standard formats, use [`parse_time()`](/reference/functions/parse_time)
