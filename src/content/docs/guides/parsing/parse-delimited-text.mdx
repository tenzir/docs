...
The examples use <Op>from_file</Op> with a [parsing
...
Use <Op>read_delimited</Op> when records use separators other than newlines.
...
See <Integration>microsoft/windows-event-logs</Integration> for a complete
example.
...
Use <Op>read_ndjson</Op> with `unflatten_separator` to convert dotted keys into
nested records:
...
For regular JSON arrays or objects, use <Op>read_json</Op> instead.
...
Use <Op>read_csv</Op> to parse the file with automatic header detection:
...
For tab-separated or space-separated data, use <Op>read_tsv</Op>
or <Op>read_ssv</Op>. For custom delimiters, use <Op>read_xsv</Op>.
...
Use <Op>read_kv</Op> to parse each line as key-value pairs:
...
Use <Op>read_cef</Op> to parse security events:
...
For IBM QRadar logs, use <Op>read_leef</Op>.
...
Use <Op>read_syslog</Op> to parse each line:
