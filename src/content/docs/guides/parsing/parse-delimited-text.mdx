...
Use <Op>read_lines</Op> to split a byte stream on
...
Use <Op>read_delimited</Op> when records use
...
See <Integration>microsoft/windows-event-logs</Integration> for a
complete example.
...
Use <Op>read_ndjson</Op> with `unflatten_separator`
...
For regular JSON arrays or objects, use <Op>read_json</Op>
instead.
...
Use <Op>read_csv</Op> to parse the file with automatic
header detection:
...
<Op>read_tsv</Op> or
<Op>read_ssv</Op>. For custom delimiters, use
<Op>read_xsv</Op>.
...
Use <Op>read_kv</Op> to parse each line as key-value
pairs:
