...
<Op>from_file</Op> operator. You'll learn to read
individual files, batch process directories, and set up real-time file
monitoring.

:::caution[Transitioning from legacy operators]
We designed the <Op>from_file</Op> operator to
replace the existing <Op>from_file</Op>,

<Op>from_s3</Op>, and
<Op>from_google_cloud_storage</Op> operators. While we still support
these legacy operators, <Op>from_file</Op> provides a
...
releases of <Op>from_file</Op>.
:::

## Basic file reading

The <Op>from_file</Op> operator handles various file
...
Access <Integration>microsoft/azure-blob-storage</Integration> using `abfs://` URLs:
