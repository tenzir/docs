...
- [`to_http`](/reference/operators/to_http) is an **output** operator that
  sends all events from one invocation as a single HTTP request. Use it for
  webhooks and HTTP-based ingestion APIs, and wrap it in `every` when you want
  time-based batches.
...
operator invocation in one request.
