...
- [`to_http`](/reference/operators/to_http) is an **output** operator that
  sends events as HTTP requests. Use it for webhooks and HTTP-based ingestion
  APIs.
...
The `to_http` operator uses `post` by default.
...
stores the error as a string in that field and continues emitting events:
