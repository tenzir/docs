...
1. **Handle errors gracefully**. Use `error_field` in `from_http` to check
   status codes and implement fallback logic.
1. **Monitor API usage**. Track response times and error rates for
   performance.
1. **Prefer built-in pagination modes when available**. Use `paginate="link"`
   for APIs that support RFC 8288 `Link` headers or `paginate="odata"` for OData
   collection envelopes instead of writing custom lambda expressions.
