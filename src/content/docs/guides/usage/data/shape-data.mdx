...
TQL comes with numerous transformation [operators](../../operators.md) that
change the shape of events. TQL has [functions](../../functions.md) that work on
values within a single event.
...
events, use the `unroll` operator:
...
To add a single value to a list, use `append` to add it to the back and
`prepend` to add it to the front:
