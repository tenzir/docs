...
This guide walks you through generating a [TQL](/explanations/language) parser
from sample log events. You'll create a complete parser package that detects log
formats, generates parsing code, and includes test cases. Two options are
available:

- **MCP server**: Use [`make_parser`](/reference/mcp-server#make_parser) (works
  with any MCP-compatible agent)
- **TQL plugin**: Use [`/tql:make_parser`](/reference/claude-plugins/tql) (Claude
  Code only)
...
- [Claude Code](https://docs.claude.ai/en/docs/claude-code) with the [TQL
  plugin](/reference/claude-plugins/tql) installed

</TabItem>
</Tabs>

Both approaches require sample log events.
...
3. **Verify and iterate**. The above log sample resulted in a new package
   `fortinet` in your agent's current working directory. Review its contents and
   validate both the parser and the tests before you ship the package.
...
2. **Provide sample log events**. When prompted, paste your log data or provide
   a file path:
...
3. **Follow the guided phases**. The workflow walks through five phases:
   - **Phase 1**: Input analysis (format, vendor, schema)
   - **Phase 2**: Package scaffolding (directory structure, initial files)
   - **Phase 3**: Iterate and test (refine parsing with `tenzir-test`)
   - **Phase 4**: Bootstrap sampling (generate synthetic test data)
   - **Phase 5**: Summarize (document the parser)

4. **Verify and iterate**. Review the generated package contents and validate
   both the parser and the tests before you ship the package.
...
- **Map to OCSF**: Proceed with [adding an OCSF
  mapping](/guides/ai-workbench/generate-an-ocsf-mapping) to normalize the parsed
  data.
