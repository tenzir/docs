...
When the operator runs, the node first checks its local
[`tenzir.auth`](/reference/node/configuration) entries for a matching name. If
none matches, it asks the connected platform, resolves the referenced secret
through the same encrypted channel it uses for standalone secrets, and applies
the strategy-specific headers (`Authorization: Bearer …`,
