...
Our [platform command-line utility](/reference/platform-cli) supports managing
at runtime.
...
manage with the [command
line interface](/guides/platform-management/use-the-platform-cli).
...
Use the `print-auth-rule` [CLI](/reference/platform-cli) command to easily
generate auth rules in the correct format:
