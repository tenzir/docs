...
If changelog structure is invalid, `release create` and `release publish` fail
...
- **Version bump fails**: Bump flags read the most recent release manifest on
  ...
  `--patch/--minor/--major`.
- **Version file update fails**: In default `release.version_bump_mode: auto`,
  `release create` must parse every detected or configured version file.
  Disable built-in bumps (`release.version_bump_mode: off`) if your repository
  requires custom release scripting for non-standard files.
