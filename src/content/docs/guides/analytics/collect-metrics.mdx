...
to work with metrics just like regular data. Use the <Op>metrics</Op> input
...
The previous example sums the per-second CPU usage of each operator, grouped by
pipeline ID, to surface the most CPU-intensive pipelines.
