...
The following table compares common event-stream aggregation capabilities across
TQL, Microsoft KQL, and Splunk SPL:

| Capability | TQL | KQL | SPL |
| ---------- | --- | --- | --- |
| Tumbling fixed windows | ✅ `window size=1h, on=ts` | ✅ `summarize by bin(ts, 1h)` | ✅ `timechart span=1h` or `bin _time` |
| Hopping or sliding fixed windows | ✅ `every < size` | ⚠️ `sliding_window_counts()` or manual expansion | ⚠️ `streamstats time_window=...` |
| Per-key windows | ✅ Compose with `group` | ✅ `summarize ... by key, bin(ts, ...)` | ✅ `BY key` or split-by |
| Late or out-of-order event time | ✅ `tolerance` | ❌ Mostly batch semantics | ⚠️ Search-order dependent |
| Arbitrary per-window subpipeline or sink | ✅ Native subpipeline block | ❌ No direct equivalent | ❌ No direct equivalent |
| Empty window padding | ❌ Not emitted | ⚠️ Manual `range` and `union` padding | ✅ `timechart cont=true` |
| Session windows | ❌ Not supported | ✅ `row_window_session()` | ✅ `transaction maxpause` and `maxspan` |
| Custom alignment or calendar windows | ⚠️ Epoch-aligned only | ✅ `bin_at()` | ✅ `aligntime` and calendar spans |
