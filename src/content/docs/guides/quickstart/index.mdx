...
This pipelines does the following: <Op>export</Op>
references all data in the node's edge storage, and
<Op>taste</Op> samples 10 events of every unique schema.
...
auto-completes the pipeline so that it can extract events and show them to you
in the browser.
:::
...
- The <Op>set</Op> operator performs an assignment and
  creates new fields.
...
- <Op>select</Op> selects the fields to keep, but also supports an assignment
  to rename the new field in one shot.
- As you can see in the `select` operator (Suricata tab) above, TQL
  expressions have [functions](/reference/functions) like <Fn>to_lower</Fn>,
  which makes working with values a breeze.
...
<Op>publish</Op> and
<Op>subscribe</Op> operators are all you need for
...
pipeline that accepts logs via <Integration>syslog</Integration> and forwards them
