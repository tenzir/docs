...
Use <Fn>now</Fn> to get the current timestamp:
...
<Fn>time</Fn> for automatic format detection and
<Fn>parse_time</Fn> for custom formats.

### Automatic parsing with `time()`

The <Fn>time</Fn> function automatically recognizes many
...
The <Fn>time</Fn> function also supports relative time
expressions:
...
<Fn>parse_time</Fn> with an explicit format
string:
...
Get individual parts of a timestamp using <Fn>year</Fn>, <Fn>month</Fn>, <Fn>day</Fn>, <Fn>hour</Fn>, <Fn>minute</Fn>, and <Fn>second</Fn>:
...
Use <Fn>months</Fn> to create month-based durations:
...
For string inputs, use <Fn>time</Fn> with the `@`
prefix:
...
For numeric inputs, use <Fn>from_epoch</Fn> with
...
Use <Fn>since_epoch</Fn> to get the duration
...
Use <Op>every</Op> to generate events periodically, then
