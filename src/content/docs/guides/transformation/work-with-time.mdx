...
[`time()`](/reference/functions/time) for automatic format detection and
[`parse_time()`](/reference/functions/parse_time) for custom formats.

### Automatic parsing with `time()`

The [`time()`](/reference/functions/time) function automatically recognizes many
...
The [`time()`](/reference/functions/time) function also supports relative time
expressions:
...
[`parse_time()`](/reference/functions/parse_time) with an explicit format
string:
...
- **[`time()`](/reference/functions/time)**: Use for ISO 8601 timestamps, Unix
  epochs (with `@` prefix), or relative time expressions. No format string
  needed.
- **[`parse_time()`](/reference/functions/parse_time)**: Use for custom formats
  like Apache logs, US date formats, or any non-standard timestamp layout.
...
Unix timestamps represent time as seconds (or fractions) since January 1, 1970.
TQL supports both parsing and generating Unix timestamps.

### Parse Unix timestamps

For string inputs, use [`time()`](/reference/functions/time) with the `@`
prefix:
...
For numeric inputs, use [`from_epoch()`](/reference/functions/from_epoch) with
the appropriate time unit:
...
Use [`since_epoch()`](/reference/functions/since_epoch) to get the duration
...
When working with historical data, you often need to replay events with their
original timing or adjust timestamps for analysis. TQL provides two operators
for this: [`delay`](/reference/operators/delay) and
[`timeshift`](/reference/operators/timeshift).

### Adjust timestamps with timeshift

The `timeshift` operator adjusts timestamps to a new baseline while preserving
relative time differences. This is essential when you need to merge datasets
from different time periods into a coherent timeline for comparative analysis.
For example, you might want to overlay security incidents from multiple years
to identify recurring patterns, or align test data from different runs to
...
Notice how the 5.5-minute gap between login and action, and the 8.5-hour
session duration are preserved, but all timestamps now start from January 1, 2024.
...
The `delay` operator replays events according to their timestamps by
introducing sleep periods between events:
...
With `speed=0.1`, the 2-second gap between first and second events becomes 20
seconds, and the 1-second gap between second and third becomes 10 seconds. This
slower replay makes it easy to observe the delay in action.
...
This replays the logs with real-world inter-arrival times. If an event
occurred at 10:00:00 and the next at 10:00:05, the operator waits 5 seconds
between emitting them.

### Combine with periodic generation

Use [`every`](/reference/operators/every) to generate events periodically, then
replay them with modified timing:
...
This generates events every second but replays them at half speed (2 seconds
between events).
...
- **`delay`**: Introduces real-time delays between events based on their
  timestamps

Use `timeshift` when you need to analyze historical data with updated
timestamps. Use `delay` when you want to replay events with realistic timing,
such as for testing real-time processing systems or simulating live data
streams.
...
- [Transform values](/guides/transformation/transform-values) - Convert between data types
- [Parse log messages from strings](/guides/parsing/parse-log-messages-from-strings) - Parse timestamps from logs
- [Filter and select data](/guides/transformation/filter-and-select-data) - Filter by time ranges
