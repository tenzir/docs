...
JSON is the most common data exchange format. Use <Fn>print_json</Fn> to convert
any data to JSON strings:
...
:::tip[Print & Write Siblings]
Many `print_*` functions have a `write_*` sibling operator that operate on
entire events instead of values within events. The `write_*` operators return
_bytes_ instead of events. As such, you need to pair it with an [output operator
that accepts bytes](/reference/operators/#outputs).
:::

The <Fn>print_json</Fn> function has <Op>write_json</Op> as sibling operator to
format the entire event stream as JSON:
...
For streaming data, use <Fn>print_ndjson</Fn>:
...
And with the <Op>write_ndjson</Op> dual:
...
Convert tabular data to CSV with <Fn>print_csv</Fn>:
...
You get an extra header with <Op>write_csv</Op> dual:
...
For tab-separated and space-separated values, use <Fn>print_tsv</Fn>
and <Fn>print_ssv</Fn>:
...
Note the additional double quotes with <Op>write_ssv</Op> because space is
overloaded as field separator.
...
If none of the existing \*SV formats meet your needs, the <Fn>print_xsv</Fn>
function to customize field separator, list separator, and what to render for
absent values:

Use <Fn>print_xsv</Fn> for custom separators:
...
And for the entire event stream via <Op>write_xsv</Op>:
...
Convert data to YAML format with <Fn>print_yaml</Fn>:
...
Turn the entire event stream into a YAML document stream
via <Op>write_yaml</Op>:
...
Convert records to key-value format with <Fn>print_kv</Fn>:
...
Print security events in CEF format with <Fn>print_cef</Fn>:
...
:::tip[Turn print functions into write operators]
There's no `write_cef` sibling operator, but you use a combination
of <Op>select</Op> and <Op>write_lines</Op>. For example:
...
You could now add <Op>to_file</Op> or use <Op>write_lines</Op> as a printing
subpipeline in a destination operator.
:::

### LEEF (Log Event Extended Format)

Print in IBM QRadar's LEEF format with <Fn>print_leef</Fn>:
