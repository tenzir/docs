...
The [`print_json()`](/reference/functions/print_json) function has
[`write_json`](/reference/operators/write_json) as sibling operator to format
...
[`write_ssv`](/reference/operators/write_ssv) because space is overloaded as
field separator.
...
If none of the existing \*SV formats meet your needs, the
[`print_xsv()`](/reference/functions/print_xsv) function to customize field
...
There's no `write_cef` sibling operator, but you use a combination of
[`select`](/reference/operators/select) and
[`write_lines`](/reference/operators/write_lines). For example:
...
You could now add [`save_file`](/reference/operators/save_file) or use
[`write_lines`](/reference/operators/write_lines) as printing pipeline in
...
- [Parse log messages from strings](/guides/parsing/parse-log-messages-from-strings) - Parse
  different formats
- [Shape lists and records](/guides/transformation/shape-lists-and-records) - Prepare
  data for printing
- [Manipulate strings](/guides/transformation/manipulate-strings) - Work with printed output
