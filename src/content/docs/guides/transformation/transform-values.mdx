...
Use <Fn>int</Fn> and <Fn>float</Fn> to convert values to numeric types:
...
Convert strings to time values with <Fn>time</Fn>:
...
Convert strings to durations with <Fn>duration</Fn>:
...
Use <Fn>uint</Fn> for non-negative integers:
...
TQL supports IP address and subnet literals directly. You can also parse them from strings using <Fn>ip</Fn> and <Fn>subnet</Fn>:
...
Use IP inspection functions like <Fn>is_v4</Fn>, <Fn>is_v6</Fn>, <Fn>is_private</Fn>, <Fn>is_global</Fn>, <Fn>is_loopback</Fn>, and <Fn>is_multicast</Fn> to analyze addresses:
...
Get detailed IP address classification with <Fn>ip_category</Fn>:
...
Identify link-local addresses with <Fn>is_link_local</Fn>:
...
Use <Fn>abs</Fn> for absolute values and

<Fn>sqrt</Fn> for square roots:
...
Many data sources use string placeholders like `"None"`, `"N/A"`, `"-"`, or
empty strings to represent missing values. Use

<Op>replace</Op> to normalize these to actual `null`:
...
To normalize a specific field rather than all matching strings, specify the
field name:
...
Use <Fn>uuid</Fn> to create unique identifiers:
...
Use <Fn>random</Fn> to generate random values:
...
Use <Fn>env</Fn> to access environment variables:
...
Use <Fn>config</Fn> to read Tenzir's configuration:
...
Use <Fn>file_contents</Fn> to read files:
...
Use <Fn>type_of</Fn> to inspect value types. Note that this function returns detailed type information as objects:
...
Use <Fn>type_id</Fn> for type comparison:
