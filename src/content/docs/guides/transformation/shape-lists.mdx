...
Lists (arrays) contain ordered sequences of values. This guide shows you how to
work with lists — accessing elements, sorting and slicing, transforming values,
...
Use <Fn>append</Fn> and <Fn>prepend</Fn>:
...
Apply functions to each element with <Fn>map</Fn>:
...
Keep only elements that match a condition with <Fn>where</Fn>:
...
Order elements with <Fn>sort</Fn>:
...
Extract portions of a list with <Fn>slice</Fn>:
...
Chain <Fn>sort</Fn> and <Fn>slice</Fn>
...
Remove duplicates with <Fn>distinct</Fn>:
...
Use <Fn>add</Fn> for set-insertion (append only if absent)
and <Fn>remove</Fn> to delete all occurrences:
...
Combine `add()` with <Fn>distinct</Fn> when you need
...
Note: Direct list flattening is not currently supported in TQL. The <Fn>flatten</Fn> function is designed for flattening records, not lists. To work with nested lists, you would need to process them element by element.
...
Combine parallel lists with <Fn>zip</Fn>:
...
Add row numbers to your data using the <Op>enumerate</Op> operator:
