...
set descending = numbers.sort(desc=true)
```
...
set by_age = users.sort(cmp=(a, b) => a.age < b.age)
set by_name = users.sort(cmp=(a, b) => a.name < b.name)
```
...
Extract portions of a list with [`slice()`](/reference/functions/slice):

Get the first N elements:

```tql
from {xs: [1, 2, 3, 4, 5]}
set first_three = xs.slice(end=3)
```
...
```tql
from {xs: [1, 2, 3, 4, 5]}
set last_three = xs.slice(begin=-3)
```
...
set middle = xs.slice(begin=2, end=5)
```
...
```tql
from {xs: [1, 2, 3, 4, 5]}
set reversed = xs.slice(stride=-1)
```
...
set every_other = xs.slice(stride=2)
```
...
Chain [`sort()`](/reference/functions/sort) and [`slice()`](/reference/functions/slice)
to extract top or bottom elements:

```tql
from {scores: [72, 95, 88, 61, 83, 97, 56]}
set top_3 = scores.sort(desc=true).slice(end=3)
set bottom_3 = scores.sort().slice(end=3)
```
