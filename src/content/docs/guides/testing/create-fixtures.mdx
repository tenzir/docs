...
- Reuse the fixture from satellite projects. When you call
  `tenzir-test --root main-project satellite-a`, the satellite automatically
  imports the root project’s fixtures before loading its own. If a satellite
  needs specialised behaviour, it can register an additional fixture with a new
  name while still leaning on the shared implementations.
