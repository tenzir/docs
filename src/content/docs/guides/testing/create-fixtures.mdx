...
Fixtures can validate what happened during a test by declaring an assertions
dataclass. The harness calls the fixture's `assert_test` hook after the test
succeeds but before teardown, passing a typed payload that the test author
