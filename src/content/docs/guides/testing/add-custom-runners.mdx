...
class XxdRunner(runners.ExtRunner):
    """Hexdump runner that turns *.xxd files into reference artefacts."""
    ...
    def run(self, test: Path, update: bool, coverage: bool = False) -> bool:
        ...
        # 4. Update reference artefacts when requested.
...
When you invoke the harness with multiple projects in the same command, the
root project’s runners load first and satellites reuse them automatically. That
makes it simple to keep shared runners (like `xxd`) in a central project while
satellite suites focus on their own tests.
