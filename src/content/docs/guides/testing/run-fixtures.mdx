...
Standalone fixture mode and positional TEST arguments are mutually exclusive.
The harness rejects invocations that combine both:

```sh
# This produces an error:
uvx tenzir-test --fixture mysql tests/foo.tql
```

If you need to run a specific test with a specific fixture, declare the fixture
in the test's frontmatter or `test.yaml` instead and run the test normally.
