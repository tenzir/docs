...
## Step 2: install `tenzir-test`

Use `uv` (or `pip`) to pin the latest release of the harness inside a virtual
environment. The example below tracks dependencies with [uv's project
metadata](https://docs.astral.sh/uv/concepts/projects/), but you can adapt it to
your preferred Python workflow.
...
Activate the environment if you prefer to run the console script directly.
`uvx` also works when you skip activation.
...
If the command succeeds, the harness is ready to run.
...
`tests/pipelines/high-severity.tql`.
...
The command produces `tests/pipelines/high-severity.txt` with the captured
stdout.
...
Refer back to the [test framework reference](/reference/test/) whenever you need
deeper details about runners, fixtures, or configuration knobs.
