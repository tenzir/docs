...
You can combine `-m` with positional paths. The harness merges both selections,
so you can run a specific test alongside everything matching a pattern:

```sh
uvx tenzir-test tests/smoke.tql -m '*context*'
```
