...
record reference artefacts. Enable passthrough mode to pipe the `tenzir`
...
want an aggregated run. List additional project directories after `--root` to
execute them sequentially under a single invocation:
...
and run their tests in isolation. The CLI prints a compact summary showing how
