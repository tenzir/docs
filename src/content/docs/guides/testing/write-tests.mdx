...
`.stdin` file next to the test to provide this content automatically.

Create `tests/shell/echo.sh`:
...
```sh
uvx tenzir-test --update tests/shell/echo.sh
```

The harness pipes the contents of `echo.stdin` to the script's stdin. The test
outputs whatever it receives, which becomes the baseline in `echo.txt`.
