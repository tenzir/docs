...
<Op>publish</Op> and
<Op>subscribe</Op> operators. You'll learn to split
event streams for parallel processing and merge multiple sources into a single
pipeline.

## How publish/subscribe works

The <Op>publish</Op> operator sends events to a named
channel (topic) on a node. The <Op>subscribe</Op>
...
Use <Op>fork</Op> with
<Op>publish</Op> to send copies of events while
