...
using the <Op>load_balance</Op> operator. You'll
...
| --------------------- | ------------------------------- | ------------------------------ |
| <Op>load_balance</Op> | Events go to exactly one target | Throughput scaling, HA         |
| <Op>fork</Op>         | Events copied to all targets    | Parallel processing, archiving |
| <Op>publish</Op>      | Events go to all subscribers    | Fan-out to pipelines           |
