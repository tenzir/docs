---
title: Write a node plugin
---

This guide shows you how to extend Tenzir with custom operators, formats, or
connectors by writing a C++ plugin. The implementation requires the following
steps:
...
points](/explanations/architecture), each of which defines its
...
classes that inherit from `tenzir::plugin` use _virtual inheritance_ to avoid
...
Refer to the [plugin loading](/explanations/configuration) section of
