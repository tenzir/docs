...
Ensure all [dependencies](#dependencies) are available, then configure and
...
:::note
CMake defaults to a `Debug` build. When performance matters, use
...
The [Dockerfile](https://github.com/tenzir/tenzir/blob/main/Dockerfile) provides
...
Build the development image with all build-time dependencies for creating custom
plugins:
...
We use [Nix](https://nixos.org) for reproducible builds on Linux. The flake
provides all [dependencies](#dependencies):
...
You can also delegate the entire build to Nix with `nix build`, but this doesn't
support incremental builds.

For automatic shell setup via [direnv](https://direnv.net), create `.envrc`:

```
use flake
```

To build static binaries, use the static environment and presets:

```sh
nix develop .#tenzir-static
cmake --preset nix-clang-static-release
cmake --build --preset nix-clang-static-release
```
...
The minimum specified versions reflect those used in CI and manual testing.
Older versions may still work.

### SBOM

Every [release](https://github.com/tenzir/tenzir/releases) includes an
[SBOM](https://en.wikipedia.org/wiki/Software_bill_of_materials) in
[SPDX](https://spdx.dev) format listing all dependencies and their versions:
