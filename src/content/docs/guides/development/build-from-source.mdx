...
Tenzir uses [CMake](https://cmake.org) as build system. Aside from a modern
C++23 compiler, you need to ensure availability of the dependencies in the table
below.
...
<LinkCard
  title="Latest SBOM"
  href="https://github.com/tenzir/tenzir/releases/latest/download/tenzir.spdx.json"
  description="Download the Software Bill of Materials in SPDX format"
  icon="document"
/>
...
We use [Nix](https://nixos.org) for reproducible Tenzir Node builds. Nix
provides a flake to setup an environment with all dependencies. You can also
delegate the entire build process to Nix by invoking `nix build`, but be aware
that this method does not support incremental builds.
...
:::note
This Nix build environment is currently only tested on Linux.
:::

### Static Binaries

Static binaries require that the dependencies were built in static mode as
well. That means you need to use a different environment; you can enter it with:
...
:::tip[macOS]
On macOS, we recommend using Clang from the Homebrew `llvm@20`
...
The repository includes CMake presets that combine configuration options into
curated build flavors. Start by discovering the available presets:

```sh
cmake --list-presets
```

This shows presets for different compilers (Clang, GCC), build types (`Debug`,
`Release`, `RelWithDebInfo`), and environments (Nix, macOS). Pick a preset matching
your setup and configure the build:

```sh
cmake --preset <preset-name>
```

For example, to set up a release build on macOS:
...
For custom configurations without presets, invoke CMake directly:
...
For faster builds, we recommend passing `-G Ninja` to `cmake`.
...
Run end-to-end integration tests via
[tenzir-test](https://github.com/tenzir/tenzir-test):
...
example, CMake doesn't disable assertions when switching from a `Debug` to a
