...
Use top-level `categories` for the Library grouping. Use the top-level
`metadata` field for data consumed by external tools. Tenzir accepts this field
but does not interpret its contents.
...
- Put reusable parsing, cleaning, enrichment, export, and OCSF mapping logic
  under `operators/`.
- Put complete workflows with an input and output under `pipelines/`. Disable
  optional ingestion, export, and publishing pipelines by default with
  `disabled: true`.
...
- Put deterministic tests under `tests/`, including baselines for every public
  operator.
- If the package maps to OCSF, add a main mapper such as `acme::ocsf::map` and
  event-specific operators under a local namespace such as
