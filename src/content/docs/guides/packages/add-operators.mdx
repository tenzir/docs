...
The dispatcher operator initializes the shared OCSF record, keeps source residue
in `ocsf.unmapped`, and routes events to specialized mappers based on a stable
event discriminator:
...
After the package mapper runs, callers can hoist the mapped event and run the
shared OCSF helpers:
