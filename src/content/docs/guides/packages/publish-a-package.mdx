...
- Has a complete `package.yaml` with descriptive metadata, valid top-level
  `categories` (`sources`, `destinations`, `mappings`, or `contexts`), and
  inputs or contexts where needed
- Exposes reusable logic as user-defined operators under `operators/`
- Ships optional ingestion, export, and publishing pipelines disabled by default
