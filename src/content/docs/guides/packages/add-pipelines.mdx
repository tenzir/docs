...
Pipelines typically use source operators to fetch and parse data, then call
UDOs from the same package to transform the parsed events:
...
The source operator handles fetching and parsing, while the package UDO handles
cleanup, mapping, and other reusable transformation logic. Use enabled pipelines
only when the package should take action as soon as it is installed. Ship
...
```tql title="pipelines/notify-webhook.tql"
---
name: Notify Webhook
description: >
...
For opt-in operation, add `disabled: true` to pipeline frontmatter. This ships
the pipeline as a template that users must explicitly enable, combining the
convenience of ready-made pipelines with predictable installation behavior.
:::
...
```tql title="pipelines/critical-router.tql"
---
unstoppable: true
restart-on-error: 1m
---
...
For flexible packages, consider shipping both: operators that provide reusable
