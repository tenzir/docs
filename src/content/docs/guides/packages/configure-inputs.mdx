...
This guide shows you how to make packages configurable with inputs. You'll learn
how to define input variables, use templating syntax, and provide values during
installation.

## What are inputs

**Inputs** are template variables that Tenzir replaces with user-provided values
when you install a package. They allow package definitions to remain independent
...
Inputs without a default value are required—users must provide a value during
installation.

## Use inputs in TQL files

Reference inputs using the `{{ inputs.input-id }}` syntax. Tenzir replaces these
references with configured values when installing the package.

### In pipelines

```tql title="pipelines/periodic-update.tql"
---
...
Users provide input values during [package installation](/guides/packages/install-a-package).

### In operators

```tql title="operators/fetch.tql"
from_http "{{ inputs.api_endpoint }}"
parse_json
```
...
To write double curly braces literally in your TQL, use the syntax
...
Use the [`package::add`](/reference/operators/package/add) operator with an
`inputs` record:
...
For IaC-style deployments, create a `config.yaml` file next to the
`package.yaml`:
...
The node reads this configuration automatically when loading the package from
