...
Use `-m`/`--match` to select tests by name pattern instead of path:
...
You can combine paths and patterns -- the harness merges both selections:

```sh
uvx tenzir-test tests/normalize.tql -m '*context*'
```
