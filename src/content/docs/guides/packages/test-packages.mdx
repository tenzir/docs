...
OCSF mapping tests should exercise the package mapper, hoist the mapped OCSF
record, and run the shared OCSF helpers before comparing against a baseline.
This verifies both your mapping logic and the final schema shape that downstream
pipelines consume.

```tql title="tests/ocsf/auth.tql"
from_file env("TENZIR_INPUT") {
  read_json
}
acme::ocsf::map
...
<Op>ocsf::cast</Op> to validate the mapped event against the OCSF schema. Drop
or replace non-deterministic fields before the comparison, such as processing
timestamps created with <Fn>now</Fn>.
