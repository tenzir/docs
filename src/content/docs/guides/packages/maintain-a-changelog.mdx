...
`release version`, and `release publish` when you omit the version argument.
...
without consuming those entry files. You can re-run the command to continue the
same RC series while you iterate on the release.
...
### Promote or bypass an existing release candidate

Once release candidates exist for a stable target, choose one of these stable
release flows:
...
Use the plain version-less command as the default follow-up to `--rc`. Use
`--from` only when the stable release must match a specific tested candidate
exactly. Use `--current-unreleased` when you want to cut the stable release
from the current backlog instead of the latest RC snapshot.
...
If you omit the version, `tenzir-ship` publishes the latest stable release:
...
need to add `--prerelease` or `--no-latest` for RC tags.

### Trigger the bundled GitHub Actions workflow

The repository's `release.yaml` workflow mirrors the CLI options. Use `rc: true`
...
Promote an exact candidate by pairing `version` with `source-release`:

```sh
gh workflow run release.yaml \
  -f intro="Ship the tested candidate." \
  -f title="Stable release" \
  -f version=v5.4.0 \
  -f source-release=v5.4.0-rc.2
```

Bypass existing candidates with `current-unreleased`:

```sh
gh workflow run release.yaml \
  -f intro="Ship the current backlog." \
  -f title="Stable release" \
