...
Use the plain version-less command as the default follow-up to `--rc`.
Promoting to stable closes the RC cycle and removes that cycle's
`vX.Y.Z-rc.N` manifests from `releases/`. If you need to leave the RC cycle and
ship a different stable release instead, pass that new explicit version, such
as `v5.4.1` or `v5.5.0`. Avoid manual bump flags when you mean to promote the
active RC because promotion only happens through the version-less command.
