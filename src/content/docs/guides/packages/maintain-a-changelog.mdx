...
Use the plain version-less command as the default follow-up to `--rc`. If you
must pass an explicit stable version, use the active RC's stable base so the
...
Replace `<version>` with the `tenzir-ship` release tag you want to pin to, for
...
When an RC is already outstanding, trigger the same workflow without `rc` to
promote the latest candidate automatically. If you need an exact override, set
`version` to the active RC's stable base:
