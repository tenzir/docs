...
1. <Op>from</Op>: A void-to-events input operator that
reads events from a URI.
2. <Op>where</Op>: An events-to-events transformation
operator that filters events matching a predicate.
3. <Op>to</Op>: An events-to-void output operator the writes
to the specified URI.

The <Op>from</Op> and <Op>to</Op>
...
to the `.parquet` extension. You can also write the specific operators for these
operations yourself:
...
1. <Op>load_kafka</Op>: A void-to-events input
...
5. <Op>save_zmq</Op>: A bytes-to-void output
