...
If you need to leave the RC cycle and ship a different stable release instead,
pass that new explicit version.
...
Use the version-less command as the default follow-up to `--rc`. Promoting to
stable closes the RC cycle and removes that cycle's `vX.Y.Z-rc.N` manifests
from `releases/`. If you need to leave the RC cycle and ship a different stable
release instead, pass that new explicit version. Avoid manual bump flags when
you mean to promote the active RC because promotion only happens through the
version-less command.
...
latest candidate automatically. To leave the RC cycle and ship a different
stable release instead, set `version` to that new stable target. Once an RC
exists, either keep using `rc: true` to continue that series or run the stable
workflow without `rc` to promote the latest candidate.
