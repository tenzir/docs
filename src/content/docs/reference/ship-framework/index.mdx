...
To expose the bundled release workflow in your own repository, add a small
wrapper file such as:
...
Replace `<version>` with the `tenzir-ship` release tag you want to pin to.

That `.github/workflows/release.yaml` wrapper mirrors the CLI release flow.
Alongside the required `intro` and optional `title`, it accepts these
