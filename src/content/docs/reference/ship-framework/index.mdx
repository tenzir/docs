...
If you need an exact override, pass the explicit stable version that matches the
active RC base to keep using that promotion path.
...
# Or promote the exact stable version that matches the active RC base
tenzir-ship release create v1.2.3 --yes
```

Use the version-less command as the default follow-up to `--rc`. If you must
pass an explicit stable version, use the active RC's stable base so the
promotion reuses the RC snapshot. Promoting to stable closes the RC cycle and
removes that cycle's `vX.Y.Z-rc.N` manifests from `releases/`. Avoid manual
bump flags while an RC is outstanding because they start a different stable
resolution path instead of promoting the active candidate.
...
For a complete `.github/workflows/release.yaml` wrapper example that calls
`tenzir/ship/.github/workflows/reusable-release.yaml@<version>`, see the guide
for [maintaining a changelog](/guides/packages/maintain-a-changelog). That
wrapper mirrors the CLI release flow. Alongside the required `intro` and
optional `title`, it accepts these release-shape inputs:
...
latest candidate automatically. You can also set `version` to the active RC's
stable base to keep using the same promotion snapshot when you need an exact
override. Once an RC exists, either keep using `rc: true` to continue that
series or run the stable workflow without `rc` to promote the latest
candidate.
