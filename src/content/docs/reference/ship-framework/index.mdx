...
# Promote an exact release candidate to stable
client.release_create(
    version="v1.0.0",
    source_release="v1.0.0-rc.1",
    assume_yes=True,
)

# Publish defaults to the latest stable release
...
| `--from <version>`    | Promote the specified release candidate into a stable tag  |
| `--current-unreleased` | Create a stable release from the current unreleased queue |
...
To override the inferred base, prefer a manual bump such as `--minor`. Pass an
explicit stable `version` only when you need an exact base.
...
#### Promote or bypass a release candidate

After release candidates exist for a stable target, you must choose whether to
promote a candidate snapshot exactly or cut the stable release from the current
unreleased queue:

```sh
# Promote the exact release-candidate snapshot
tenzir-ship release create v1.2.3 --from v1.2.3-rc.2 --yes

# Ignore existing release candidates and use the current unreleased queue
tenzir-ship release create --current-unreleased --yes
```
...
| Option             | Description                                             |
| ------------------ | ------------------------------------------------------- |
| `version`          | Release version (defaults to the latest stable release) |
...
you omit `version`, the command resolves the latest stable release. Release
candidates never replace this default.
...
# 4. Publish with commit and tag (defaults to the latest stable release)
...
# 4. Publish (defaults to the latest stable release)
tenzir-ship release publish --tag --yes
```

Both workflows are version-agnostic after step 1. The `release publish` command
defaults to the latest stable release when no version is specified.
...
The bundled `.github/workflows/release.yaml` workflow mirrors the CLI release
flow. Alongside the required `intro` and optional `title`, it accepts these
release-shape inputs:
...
| `source-release`     | Promote the specified release candidate into a stable target  |
| `current-unreleased` | Create a stable release from the current unreleased queue     |

Use `rc: true` for iterative release-candidate builds, `version` together with
`source-release` to promote an exact candidate, and `version` together with
`current-unreleased: true` when you need to bypass existing candidate
snapshots.

For normal stable releases, prefer leaving both `bump` and `version` unset so
the workflow auto-resolves the release version from the changelog.

If an outstanding RC exists, that same version-less workflow promotes the
latest candidate automatically. Add `current-unreleased: true` only when you
want to bypass that candidate snapshot and cut the stable release from the
current unreleased queue instead.
...
- **Version bump fails** – Bump flags resolve from the latest stable release on
  ...
  `--patch/--minor/--major`. If release candidates already exist for the target
  stable version, promote one with `--from` or bypass them with
  `--current-unreleased`.
