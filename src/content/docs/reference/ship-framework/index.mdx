...
project root and parent directory for `package.json`, `pyproject.toml`,
`project.toml`, and `Cargo.toml` files and updates their version fields to match
the release version.

Files without a static version field are skipped gracefully. For example,
`pyproject.toml` files with `dynamic = ["version"]` and workspace `Cargo.toml`
files are ignored in auto mode.
