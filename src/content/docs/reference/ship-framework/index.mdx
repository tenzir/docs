...
For nested changelog projects (where the changelog lives in a `changelog/`
subdirectory), the parent directory is also searched.

The version written to files strips the `v` prefix from the release label, so
`v1.2.3` becomes `1.2.3` in the manifest file.
