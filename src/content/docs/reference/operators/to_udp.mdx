...
`to_udp` consumes events directly. It does not accept byte streams from
`write_*` operators; use the `message` expression to choose the string, blob, or
record that should become the datagram payload.
...
An expression that is evaluated once per input event. The result must produce
