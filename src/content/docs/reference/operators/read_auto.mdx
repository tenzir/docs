...
input bytes
    │
    ▼
probe the first bytes (up to max_probe_bytes)
    │
    ▼
dry-run every reader's parser on the probe
    │
    ├─ exactly one best match ──▶ selected reader parses the stream
    ├─ no capable reader ───────▶ fallback reader, or error
    └─ tie in specificity ──────▶ error (ambiguous input)
```
