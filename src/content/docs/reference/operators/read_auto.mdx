...
1. **Capability**: Every reader dry-runs its actual parser on the probe. For
   example, YAML detection runs the YAML parser and requires a map document
   that <Op>read_yaml</Op> would turn into an event, and CSV detection
   tokenizes complete lines with the reader's quoting rules and requires a
   stable number of fields. A reader only becomes a candidate when it would
   accept the probed input.
