...
(zero-byte objects with keys ending in `/`) encountered while listing. These
markers are artifacts from some S3 tools and can accumulate over time. Removing
them does not affect other files.
