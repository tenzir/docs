...
The `<database>.<table>` notation can be used to also specify a table. If no `<database>`
is provided, ClickHouse uses the current database selected by the URI or server defaults.
...
- `"create_append"` Creates a table and database if they do not exist. Appends
