...
3. **Show mode**: List database metadata using the `show` parameter.
   When `show="columns"`, also set `table` to the table name.

Internal metadata queries, such as resolving tracking columns for live mode,
validate user-provided table and column names against a safe list and use MySQL
prepared statements to safely bind values.
...
The name of the table to read from. This is mutually exclusive with `sql`.
When `show="columns"`, set `table` to the table name.
...
The operator maps MySQL types to <Reference>types</Reference> as follows:
