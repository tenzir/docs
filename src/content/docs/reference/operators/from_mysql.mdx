...
When provided, the URI supplies connection parameters. Providing credentials in
both the URI and as explicit parameters (`user`, `password`) is an error.
...
Retrieve database metadata. This is mutually exclusive with `sql`.
...
When using `show="columns"`, the `table` parameter specifies which table to
inspect (not as a query mode).
...
| `JSON`                        | `string`    |                        |
| `ENUM`                        | `string`    | Enum value as text     |
...
The operator performs one-shot queries. Continuous polling for newly inserted
rows (e.g., tracking by incremental row ID) is not yet supported.
