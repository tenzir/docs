...
Use log group ARNs with `mode="live"`. Live mode accepts at most 10 log
groups. Use a list only with `mode="live"`; historical modes require exactly one
log group.
...
more streams. In live mode, `stream` requires exactly one `group`. Live and
search modes accept at most 100 streams.

This option is mutually exclusive with `stream_prefix`.

### `stream_prefix = string (optional)`

Non-empty prefix for selecting log streams in `mode="live"` or
`mode="search"`.
