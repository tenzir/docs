...
```tql
http url, paginate=(x => x.next_url?)
```
...
This is useful for APIs that include pagination URLs in the `Link` response
header with `rel=next`, such as GitHub, GitLab, and Jira.
