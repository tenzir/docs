...
The `from_amqp` operator is an [AMQP](https://www.amqp.org/) 0-9-1 client that
...
The URL is required. Values in the URL override corresponding entries from
`options`.
...
If `true`, the queue is exclusive to the current connection and is deleted when
...
If `true`, the broker doesn't send messages to the connection that published
them.
...
If `true`, the broker expects acknowledgements for messages. If `false`, the
