...
The <op>from_clickhouse</op> operator issues a query to a ClickHouse server. You
...
:::note
ClickHouse omits `ALIAS` and `MATERIALIZED` columns from `SELECT *`. The
`table` option therefore does not include such columns. To read them, use `sql`
