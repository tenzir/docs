...
<Op>nics</Op> operator lists all available interfaces.
...
Defaults to `false`. When `false`, the data field contains a UTF-8 string and
invalid UTF-8 datagrams are dropped with a warning. When `true`, the data field
contains raw bytes as a blob.
