...
| Pattern                       | Use Case                                       |
| ----------------------------- | ---------------------------------------------- |
| `[time.round(1d)]`            | Time-series data with date-range queries       |
| `[time.round(1h)]`            | High-volume streams needing hourly granularity |
| `[time.round(1d), source]`    | Multi-source ingestion with source filtering   |
| `[time.round(1d), class_uid]` | OCSF events filtered by event class            |
...
to_databricks
  ...
  flush_interval=15m,
  file_size=1Gi,
  partition_by=[time.round(1d), source_type]
```
