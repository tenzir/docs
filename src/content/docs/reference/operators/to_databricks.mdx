...
- `external_delta` _(planned)_: Writes Delta tables directly to customer-managed
...
with dictionary encoding, run-length encoding, and Snappy compression for
...
```tql
load_file "/var/log/zeek/conn.log"
read_zeek_tsv
to_databricks
