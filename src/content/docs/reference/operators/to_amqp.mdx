...
The `to_amqp` operator is an [AMQP](https://www.amqp.org/) 0-9-1 client that
...
If `true`, the broker returns an unroutable message instead of silently dropping
it.
...
If `true`, the broker returns an undeliverable message when no consumer can
