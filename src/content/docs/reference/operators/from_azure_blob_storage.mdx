...
When using `remove=true`, the operator also cleans up directory marker objects
(zero-byte blobs with names ending in `/`) encountered while listing. These
markers are artifacts from some Azure tools and can accumulate over time.
Removing them does not affect other files.
