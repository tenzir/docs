...
| -------- | -------------------------------------- | --------------- | -------- | ------------------------------------------- |
| `tenzir` | `tenzir -f <test>`                     | `.tql`          | `.txt`   | Evaluate pipelines and capture stdout.      |
| `python` | Execute with the active Python runtime | `.py`           | `.txt`   | Run Python-based tests that emit baselines. |
| `custom` | `sh -eu <test>` via the harness helper | `.sh`           | varies   | Drive shell-based scenarios with fixtures.  |
...
2. Default suffix mapping applies when no runner explicitly claims an extension:
   `.tql → tenzir`, `.py → python`, other suffixes fall back to `tenzir`.
...
`register_alias()` to publish alternate names. The [runner guide](/guides/testing/add-custom-runners)
contains a full example (`XxdRunner`).
...
| ---------- | --------------- | --------- | ------------------------------------------------------ |
| `runner`   | string          | by suffix | Runner name (`tenzir`, `python`, custom).              |
...
Implement fixtures in `fixtures/` and register them with decorators:

- `@tenzir_test.startup()` (or `@startup(replace=True)`) – Run before the test.
  Return a mapping of environment variables for the test process.
- `@tenzir_test.teardown()` – Receive the environment mapping and perform
  cleanup.
- Fixtures can also return `FixtureHandle` objects to bundle teardown logic.

The [fixture guide](/guides/testing/create-fixtures) demonstrates an HTTP echo
server that exposes `HTTP_FIXTURE_URL` and tears down cleanly. When your fixture
needs the built-in node, call `fixtures.require("node")` to fail fast.
...
Place scripts in `custom/` (with `.sh`) to run them under `bash -eu`. During
execution the harness sets:

- `TENZIR_TESTER_CHECK_PORT`
- `TENZIR_TESTER_CHECK_UPDATE`
- `TENZIR_TESTER_CHECK_PATH`

Use these values to coordinate multi-step assertions or communicate with helper
processes.
