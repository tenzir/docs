...
Blobs represent raw binary data. Use them for handling non-textual data like
network packets, encrypted payloads, or file contents.

Read [why TQL has binary blob
types](/reference/types/#why-binary-blob-types) for details.
...
<Explanation>secrets</Explanation> protect sensitive values like authentication
...
Create secrets using the <Fn>secret</Fn> function or
...
Trying to access a field that does not exist in an event will raise a warning
...
The `take` expression transfers a field's value and removes the original field
in one atomic operation. Use the `take` keyword in front of a field to relocate
...
Use `take` in assignments to avoid separate delete operations:
...
The previous `move` expression keyword is deprecated. Use `take` in new
pipelines.

In addition to the `take` keyword, there exists a
<Op>move</Op> operator that is a convenient alternative
when relocating multiple fields. For example, this
sequence of assignments with the `take` keyword:
...
can be rewritten succinctly with the <Op>move</Op>
operator:
...
- Replaces the deprecated `move` expression keyword
- Different from the <Op>move</Op> operator that is a
  statement
  :::
...
The `and` and `or` operators use short-circuit evaluation: they evaluate the
second operand only if necessary. This means that if the first operand is
sufficient to determine the result, the second operand is not evaluated.
:::
...
Reference previously defined [`let`](/reference/statements#let)
bindings using `$`-prefixed names:
