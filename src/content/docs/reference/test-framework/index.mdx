...
- `capability-unavailable` — skip when a required capability (declared via
  [`requires`](#capability-requirements)) is missing from the Tenzir build.
...
The `requires` key declares capabilities that the Tenzir build must provide
for the suite to run. It is only allowed in directory-level `test.yaml` files.
Currently, the only supported category is `operators`:
...
Before running the suite, the harness probes the Tenzir binary for each listed
operator. If any are missing and the suite carries
...
Capability probes are runner-aware. The TQL runner checks operators by querying
`plugins | where name == "<operator>"`. Runners that do not implement
capability probes report all requirement categories as unsupported, which
raises a hard error. This means `requires` is currently effective only for
suites whose tests use the TQL runner.
