...
- `-m`, `--match`: Run tests whose relative path matches a glob pattern
  ...
  positional `TEST` paths, the two selections are merged (union). Patterns
  are case-sensitive.
...
When you combine positional `TEST` paths with `-m` patterns, the harness merges
both selections (union). This lets you run a named test alongside everything
matching a pattern:

```sh
uvx tenzir-test tests/foo.tql -m '*context*'
```
