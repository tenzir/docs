...
- `-p`, `--passthrough`: Stream raw stdout/stderr to the terminal instead of
  comparing against reference artefacts. The harness forces single-job
...
in one go. The directory given to `--root` acts as the **root project**; all
other directories are treated as **satellites**:
...
- You can target subsets inside each project with additional positional
  arguments (`tenzir-test --root main secondary tests/smoke`). When you skip
  `--root` entirely and only list satellite directories, the harness runs those
  satellites in isolation.
...
mode. If you need to branch on the current behaviour, call
...
- `HarnessMode.COMPARE` – default behaviour; compare actual output with stored baselines.
