...
to the subprocess stdin. The harness also exposes the file path via
`TENZIR_STDIN`:
...
A shell script that reads from stdin:
...
automatically. Tests can combine `.stdin` with `.input` when they need both
stdin content and a test-specific input file:
...
The harness sets `TENZIR_STDIN` only when a matching `.stdin` file exists.
