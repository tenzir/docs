...
accept structured assertion payloads from tests. Tests declare payloads under
`assertions.fixtures.<name>` in frontmatter:
...
The harness constructs a typed instance and invokes the fixture's `assert_test`
hook after the test succeeds but before teardown. Omitting the `assertions`
block skips the hook. A raised exception fails the test. For suite tests, the
hook runs after each member rather than once at the end.
