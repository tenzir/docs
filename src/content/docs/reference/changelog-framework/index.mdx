...
automatically. The CLI names entry files using the slugified title (e.g.,
`my-feature.md`).
...
The first invocation of `tenzir-changelog add` scaffolds `config.yaml`
...
The singular form is shorthand for single values and is normalized to the plural
form internally.
