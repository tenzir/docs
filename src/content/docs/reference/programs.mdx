...
<Op>serve</Op> to turn the dataflow into a REST API,
...
1. **Closed subpipelines** (void-to-void): Complete programs that run
   independently, used by operators like <Op>every</Op>
   and <Op>subscribe</Op>.

2. **Parsing subpipelines** (bytes-to-events): Transform raw bytes into
   structured events, used by input operators like

   <Op>from_file</Op> and <Op>from_http</Op>.

3. **Printing subpipelines** (events-to-bytes): Transform structured events into
   raw bytes, used by output operators like <Op>to_file</Op> and
   <Op>to_http</Op>.

### Closed subpipelines

The <Op>every</Op> operator executes a closed subpipeline
...
Input operators like <Op>from_file</Op> or

<Op>from_http</Op> that read raw bytes use parsing
...
<Op>from_kafka</Op> or
<Op>accept_udp</Op>, don't take a parsing subpipeline
...
<Op>import</Op> operator implicitly performs a network
