...
TQL **programs** compose [statements](/reference/statements) into complete data
processing workflows that can execute. Valid TQL programs adhere to the
following rules:
...
auto-completetion takes place with an output operator: The web app
appends <Op>serve</Op> to turn the dataflow into a REST API, allowing your browser to
access it by routing the data through the platform.
:::
...
Subpipelines are nested pipelines that an outer operator executes based on its
own semantics. You define subpipelines within a block of curly braces (`{}`).
They are a first-class part of TQL and the preferred v6 pattern for separating
transport from parsing or printing.
...
1. **Closed subpipelines** (void-to-void): Complete programs that run
   independently, used by operators like <Op>every</Op> and <Op>subscribe</Op>.

2. **Parsing subpipelines** (bytes-to-events): Transform raw bytes into
   structured events, used by input operators like <Op>from_file</Op>
   and <Op>from_http</Op>.

3. **Printing subpipelines** (events-to-bytes): Transform structured events into
   raw bytes, used by output operators like <Op>to_file</Op>
   and <Op>to_http</Op>.

### Closed subpipelines

The <Op>every</Op> operator executes a closed subpipeline at regular intervals:
...
Input operators like <Op>from_file</Op> or <Op>from_http</Op> that read raw
bytes use parsing subpipelines to convert bytes into events. This pattern
separates _where_ data comes from (the outer operator) from _how_ it's parsed
(the subpipeline):
...
Operators that produce events directly, like <Op>from_kafka</Op>
or <Op>accept_udp</Op>, don't take a parsing subpipeline because the data format is
...
Use a slash-star (`/*`) to start a multi-line comment and a star-slash (`*/`) to
end it.
...
- <Op>where</Op>: Filters one event at a time
- <Op>select</Op>: Transforms fields immediately
- <Op>drop</Op>: Removes fields as events flow

**Blocking operators** need all input before producing output:

- <Op>sort</Op>: Must see all events to order them
- <Op>summarize</Op>: Aggregates across the stream
- <Op>reverse</Op>: Needs complete input to reverse order
...
TQL pipelines can span network boundaries seamlessly. For example,
the <Op>import</Op> operator implicitly performs a network connection based on where
it runs. If the `tenzir` binary executes the pipeline, the executor establishesa
transparent network connection. If the pipeline runs within a node, the executor
passes the data directly to the next operator in the same process.
